 ...
 */
class EPRF_Add_Ons_Page {

	public function __construct() {
		add_filter( EPRF_KB_Core::EPRF_KB_LICENSE_FIELD, array( $this, 'display_license_status_update') );
		add_action( 'wp_ajax_eprf_handle_license_request', array( $this, 'handle_license_request' ) );
		add_action( 'wp_ajax_nopriv_eprf_handle_license_request', array( 'EPRF_Utilities', 'user_not_logged_in' ) );
	}
	...
	public function display_license_status_update( $output ) {
		...
		$output .= '<div class="ekb-add-on-title">'
					.'<h2>'. EPRF_License_Handler::PLUGIN_NAME . '</h2>' .
			       '</div>'.
		           '<div class="ekb-license-check" id="eprf_license_check">' . __( 'Updating license status. Please wait.', 'echo-knowledge-base' ) . '</div>' .
		           '<div id="eprf-ajax-in-progress" style="display:none;">' . esc_html__( 'Updating license...', 'echo-knowledge-base' ) . '</div>';
		$output .= '</li>';
	...
	public function handle_license_request() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			EPRF_Utilities::ajax_show_error_die( __( 'You do not have permission to view license info.', 'echo-knowledge-base' ) );
		}

		$eprf_instance = eprf_get_instance();
		if ( empty($eprf_instance) || ! $eprf_instance instanceof Echo_Article_Rating_And_Feedback ) {
			EPRF_Utilities::ajax_show_error_die( __( 'Failed to initialize (M99).', 'echo-knowledge-base' ) );
		}

		$command = EPRF_Utilities::post('command');
		$command = ! empty($command) && in_array( $command, array( 'get_license_info', 'save' ) ) ? $command : '';
		if ( empty($command) ) {
			EPRF_Utilities::ajax_show_error_die( __( 'Unknown licensing command: ', 'echo-knowledge-base' ) . $command );
		}

		if ( empty( $eprf_instance->license_handler ) ) {
			EPRF_Utilities::ajax_show_error_die( __( 'Failed to initialize (L99)', 'echo-knowledge-base' ) );
		}

		$license_handler = $eprf_instance->license_handler;
	 ...
	 *
	 * @param EPRF_License_Handler $license_handler
	 * @param $license_data
	...
	private function get_license_state( $license_handler, $license_data ) {
		...
		if ( ! empty($license_error) && $license_handler->is_connection_error( $license_error ) ) {
			EPRF_Logging::add_log("Could not contact licensing server.", ( isset($license_data->license) ? $license_data->license : '' ) );
			$cached_info = $license_handler->get_cached_info();
		...
		EPRF_Utilities::save_wp_option('eprf_last_license_check', array(), true);
	...
	private function license_fields( $status, $status_msg, $expiry_date, $latest_add_on_version, $license_key ) {
		...
		<div class="ekb-license-key">
			<ul>
				...
				<li><input type="text" name="<?php echo EPRF_License_Handler::LICENSE_KEY_OPTION_ID; ?>" id="<?php echo EPRF_License_Handler::LICENSE_KEY_OPTION_ID; ?>"
				...
				<li><span class="eprf_license_action"><input id="eprf_save_btn" class="primary-btn" type="button" name="eprf_save_btn" value="<?php echo  __( 'Save', 'echo-knowledge-base' ); ?>"/>
			</ul>
		</div>
		...
		<div class="ekb-license-status">
			<ul>
				...
				<li><span id="eprf_license_key_status"><?php echo wp_kses_post( $status_msg ); ?></span></li>
			</ul>
		</div>
		...
		<div class="ekb-license-expiry">			<?php
			...
			$expiry_msg = empty($expiry_msg) ? EPRF_Utilities::get_formatted_datetime_string( $expiry_date ) : $expiry_msg;			?>
		...
		<div class="ekb-license-version">			<?php
			...
			$new_version_available = version_compare( Echo_Article_Rating_And_Feedback::$version, $latest_add_on_version, "<" );
			...
			<ul>
				...
				<li><?php _e( 'Installed:', 'echo-knowledge-base' ); ?> <?php echo Echo_Article_Rating_And_Feedback::$version; ?></li>
				...
				<li>					<?php
					...
					if ( $new_version_available && $status == 'valid' ) {
						$name = plugin_basename( Echo_Article_Rating_And_Feedback::$plugin_file );
