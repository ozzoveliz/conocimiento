 ...
 */
class EPRF_Utilities {

	static $wp_options_cache = array();
	static $postmeta = array();

	const EPRF_ADMIN_CAPABILITY = 'manage_options';
	 ...
	 *
	 * For example: EPRF_Utilities::ajax_show_error_die( EPRF_Utilities::report_generic_error( 411 ) );
	 *
	...
	public static function sanitize_get_id( $id ) {
		...
		if ( empty( $id) || is_wp_error($id) ) {
			EPRF_Logging::add_log( 'Error occurred (01)' );
			return new WP_Error('E001', 'invalid ID' );
		}

		if ( is_array( $id) ) {
			if ( ! isset( $id['id']) ) {
				EPRF_Logging::add_log( 'Error occurred (02)' );
				return new WP_Error('E002', 'invalid ID' );
			}

			$id_value = $id['id'];
			if ( ! self::is_positive_int( $id_value ) ) {
				EPRF_Logging::add_log( 'Error occurred (03)', $id_value );
		...
		if ( ! self::is_positive_int( $id ) ) {
			EPRF_Logging::add_log( 'Error occurred (04)', $id );
	...
	public static function retrieve_and_sanitize_form( $form, $all_fields_specs ) {
		...
		foreach( $submitted_fields as $submitted_key => $submitted_value ) {
			...
			if ( $field_type == EPRF_Input_Filter::WP_EDITOR ) {
				$sanitized_fields[$submitted_key] = wp_kses( $submitted_value, self::get_extended_html_tags() );

			} elseif ( $field_type == EPRF_Input_Filter::TYPOGRAPHY ) {
				$sanitized_fields[$submitted_key] = EPRF_Input_Filter::sanitize_typography( $submitted_value );

			} elseif ( $field_type == EPRF_Input_Filter::TEXT && ! empty($all_fields_specs[$submitted_key]['allowed_tags']) ) {
	 ...
	 */
	public static function ajax_verify_nonce_and_admin_permission_or_error_die( $wpnonce_name='_wpnonce_eprf_ajax_action', $context='' ) {
		...
		if ( ! EPRF_KB_Core::is_user_access_to_context_allowed( $context ) ) {
	...
	public static function get_wp_option( $option_name, $default, $is_array=false, $return_error=false ) {
		...
		if ( $return_error && $option === null && ! empty($wpdb->last_error) ) {
			...
			EPRF_Logging::add_log( "DB failure: " . $wpdb_last_error, 'Option Name: ' . $option_name );
	...
	public static function save_wp_option( $option_name, $option_value, $sanitized ) {
		...
		if ( $result === false ) {
			EPRF_Logging::add_log( 'Failed to update option', $option_name );
	...
	public static function get_postmeta( $post_id, $meta_key, $default, $is_array=false, $return_error=false ) {
		...
		if ( $return_error && $option === null && ! empty($wpdb->last_error) ) {
			$wpdb_last_error = $wpdb->last_error;   // add_log changes last_error so store it first
			EPRF_Logging::add_log( "DB failure: " . $wpdb_last_error, 'Meta Key: ' . $meta_key );
			return new WP_Error(__( 'Error occurred', 'echo-knowledge-base' ), $wpdb_last_error);
		}
	...
	public static function save_postmeta( $post_id, $meta_key, $meta_value, $sanitized ) {
		...
		if ( $result === null && ! empty($wpdb->last_error) ) {
			$wpdb_last_error = $wpdb->last_error;   // add_log changes last_error so store it first
			EPRF_Logging::add_log( "DB failure: " . $wpdb_last_error );
			return new WP_Error(__( 'Error occurred', 'echo-knowledge-base' ), $wpdb_last_error);
		}
		...
		if ( empty($result) ) {
			if ( false === $wpdb->query( $wpdb->prepare( "INSERT INTO $wpdb->postmeta (`meta_key`, `meta_value`, `post_id`) VALUES (%s, %s, %d)", $meta_key, $serialized_value, $post_id ) ) ) {
				EPRF_Logging::add_log("Failed to insert meta data. ", $meta_key);
			...
			if ( false === $wpdb->query( $wpdb->prepare( "UPDATE $wpdb->postmeta SET meta_value = %s WHERE meta_key = '%s' AND post_id = %d", $serialized_value, $meta_key, $post_id ) ) ) {
				EPRF_Logging::add_log("Failed to update meta data. ", $meta_key);
				return new WP_Error( '33', __( 'Error occurred', 'echo-knowledge-base' ) );
			}
		}

		if ( $result === false ) {
			EPRF_Logging::add_log( 'Failed to update meta key', $meta_key );
	...
	public static function delete_postmeta( $post_id, $meta_key ) {
		...
		if ( false === $wpdb->query( $wpdb->prepare( "DELETE FROM $wpdb->postmeta WHERE post_id = %d and meta_key = '%s'", $post_id, $meta_key ) ) ) {
			EPRF_Logging::add_log("Could not delete post '" . self::get_variable_string($meta_key) . "'' metadata: ", $post_id);
			return false;
		}
	...
	public static function get_inline_style( $styles, $config ) {
		...
		foreach( $style_array as $style ) {
			...
			if ( count($key_value) == 2 && isset($key_value[1]) ) {
				...
				$typography_values = array_merge( EPRF_Typography::$typography_defaults, $config[$key_value[2]] );
	...
	public static function get_typography_config( $typography ) {
		...
		if ( empty( $typography ) || ! is_array( $typography ) ) {
			$typography = EPRF_Typography::$typography_defaults;
		}
	...
	public static function send_email( $message, $to_support_email='', $reply_to_email='', $reply_to_name='', $subject='' ) {
		...
		if ( empty( $message ) || strlen( $message ) > 5000 ) {
			EPRF_Logging::add_log( 'Invalid or too long email message', $message );
		...
		add_filter( 'wp_mail_content_type', array( 'EPRF_Utilities', 'set_html_content_type' ), 999 );

		// send email
		$result = wp_mail( $to_support_email, $subject, $message, $headers );

		// remove filter that allows HTML in the email content
		remove_filter( 'wp_mail_content_type', array( 'EPRF_Utilities', 'set_html_content_type' ), 999 );
	...
	public static function admin_ui_wp_kses( $html ) {
		...
		add_filter( 'safe_style_css', array( 'EPRF_Utilities', 'admin_ui_safe_style_css' ) );

		$sanitized_html = wp_kses( $html, self::get_admin_ui_extended_html_tags() );

		// disallow specific CSS styles
		remove_filter( 'safe_style_css', array( 'EPRF_Utilities', 'admin_ui_safe_style_css' ) );
