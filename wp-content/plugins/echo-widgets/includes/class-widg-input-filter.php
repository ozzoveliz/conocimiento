...
class WIDG_Input_Filter {
	...
	public function validate_and_sanitize_specs( array $input, array $specification ) {
		...
		foreach ( $input as $key => $input_value ) {
			...
			switch ( $type ) {
				...
				case self::WP_EDITOR:
					$input_value = wp_kses( $input_value, WIDG_Utilities::get_extended_html_tags() );
					break;
			...
			if ( is_wp_error( $result ) ) {
				...
				WIDG_Logging::add_log( 'Please change the value of ' . $field_spec['label'] . ' field. Current value: "' . $input_value . '" - ' . $result->get_error_message() . ', code: ' . $result->get_error_message(), $result );
	...
	private function filter_number( $number, $field_spec=array() ) {
		...
		$number_int = WIDG_Utilities::sanitize_int( $number, null );
	...
	private function filter_id( $id ) {
		$id = WIDG_Utilities::sanitize_get_id( $id );
		if ( is_wp_error($id) ) {
	...
	private function filter_typography( $value ) {
		...
		if ( ! is_array( $value ) ) {
			return new WP_Error( 'filter_not_typograghy', __( 'Error Occurred', 'echo-knowledge-base' ) . ' - ' . WIDG_Utilities::get_variable_string( $value ) );
		}

		$google_fonts = WIDG_Typography::get_google_fonts_family_list();

		if ( ! empty($value['font-family']) && ! in_array($value['font-family'], $google_fonts) ) {
			$value['font-family'] = WIDG_Typography::$typography_defaults['font-family'];
		}

		if ( ! empty($value['font-size']) && ( $value['font-size'] < 4 || $value['font-size'] > 100 ) ) {
			$value['font-size'] = WIDG_Typography::$typography_defaults['font-size'];
		}

		if ( ! in_array($value['font-size-units'], ['px', 'em']) ) {
			$value['font-size-units'] = WIDG_Typography::$typography_defaults['font-size-units'];
		}

		if ( ! empty($value['font-weight']) && ! in_array($value['font-weight'], [100, 200, 300, 400, 500, 600, 700, 800, 900]) ) {
			$value['font-weight'] = WIDG_Typography::$typography_defaults['font-weight'];
		}
	...
	public static function sanitize_typography( $typography ) {
		...
		$typography = array_merge( WIDG_Typography::$typography_defaults, $typography );

		$typography_filtered = [];
		$allowed_keys = array_keys( WIDG_Typography::$typography_defaults );
	...
	public function retrieve_and_sanitize_form_fields( $submitted_fields, $all_fields_specs, $orig_settings ) {
		...
		foreach ($all_fields_specs as $key => $spec ) {
			...
			if ( $spec['type'] == self::WP_EDITOR ) {
				$name_values += array( $key => wp_kses( $input_value, WIDG_Utilities::get_extended_html_tags() ) );
