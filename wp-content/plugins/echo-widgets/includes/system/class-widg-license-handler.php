...
class WIDG_License_Handler {
	...
	const LICENSE_KEY_OPTION_ID = 'widg_license_key';
	const WIDG_LICENSE_STATE    = 'widg_license_state';
	...
	function __construct( $_file, $_version ) {
		...
		$this->license_key = WIDG_Utilities::get_wp_option( self::LICENSE_KEY_OPTION_ID, '' );
		$this->setup_hooks();
	}
	...
	private function setup_hooks() {
		...
		add_filter( WIDG_KB_Core::WIDG_KB_ADD_ON_LICENSE_MSG, array( $this, 'get_license_status_short') );
	}
	...
	public function auto_updater() {
		...
		// set up the updater
		if ( ! class_exists( 'WIDG_EDD_SL_Plugin_Updater' ) ) {
			require_once 'class-widg-edd-sl-plugin-updater.php';
		}

		new WIDG_EDD_SL_Plugin_Updater(
	...
	public function get_cached_info() {
		$default_array = array( __( 'Unknown status.', 'echo-knowledge-base' ), '', Echo_Widgets::$version, '' );
		$widg_license_state = WIDG_Utilities::get_wp_option( self::WIDG_LICENSE_STATE, $default_array, true );
		$status_msg = isset($widg_license_state[0]) ? $widg_license_state[0] : __( 'Unknown status.', 'echo-knowledge-base' );
		$license_state = isset($widg_license_state[1]) ? $widg_license_state[1] : '';
		$latest_add_on_version = isset($widg_license_state[2]) ? $widg_license_state[2] : Echo_Widgets::$version;
		$expiry_date = isset($widg_license_state[3]) ? $widg_license_state[3] : '';
	...
	public function handle_license_command() {
		...
		if ( ! isset( $_REQUEST['_wpnonce_widg_license_key'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce_widg_license_key'], '_wpnonce_widg_license_key' ) ) {
			WIDG_Utilities::ajax_show_error_die(__( 'Security check failed.', 'echo-knowledge-base' ));
		}

		// need license key to activate/deactivate the license
		$entered_license_key = empty($_REQUEST['widg_license_key']) ? '' : sanitize_text_field( $_REQUEST['widg_license_key'] );
	...
	private function save_license( $license_key ) {
		...
		$result = WIDG_Utilities::save_wp_option( self::LICENSE_KEY_OPTION_ID, $license_key, true );
		if ( is_wp_error( $result ) ) {
			/* @var $result WP_Error */
			$message = $result->get_error_data();
			if ( empty($message) ) {
				WIDG_Utilities::ajax_show_error_die( $result->get_error_message(), __( 'Could not save settings.', 'echo-knowledge-base' ) );
			}
			WIDG_Utilities::ajax_show_error_die( $this->generate_error_summary( $result->get_error_data() ), __( 'Settings NOT saved due to following problems:', 'echo-knowledge-base' ) );
		}
	}
	...
	public function contact_license_server( $action, $license_key ) {
		...
		if ( is_wp_error( $response ) ) {
			$error_msg = '<br/>REQUEST: url => ' . $get_url . ', args => ' . WIDG_Utilities::get_variable_string( $get_args );
			$error_msg .= '<br/>RESPONSE: ' . WIDG_Utilities::get_variable_string( $response );
	...
	public function get_expiry_warning( $expiry_date ) {
		...
		$difference_in_days = WIDG_Utilities::get_days_since( '', $expiry_date );
	...
	public function get_license_state( $force_refresh=false ) {
		...
		$stored_license_data = WIDG_Utilities::get_wp_option('widg_last_license_check', array(), true, true);
		if ( is_wp_error($stored_license_data) ) {
			WIDG_Logging::add_log("Could not retrieve license data option.");
			return null;
		}
		...
		$licenses_tab_url = admin_url( 'edit.php?post_type=' . WIDG_KB_Handler::get_post_type( WIDG_KB_Core::DEFAULT_KB_ID ) . '&page=ep' . 'kb-add-ons&ep' . 'kb-tab=licenses' );
		...
		if ( ! empty($license_error) && $this->is_connection_error( $license_error ) ) {
			...
			if ( ! empty($stored_license_data['last_connection_error']) && WIDG_Utilities::get_days_since( $stored_license_data['last_connection_error'] ) > 30 ) {
		...
		if ( ! empty($add_on_data) &&  is_object( $add_on_data ) && ! empty( $add_on_data->new_version ) ) {
			...
			WIDG_Logging::add_log("Could not contact licensing server.", ( isset($add_on_data->license) ? $add_on_data->license : '' ) );
			$cached_info = $this->get_cached_info();
			$latest_add_on_version = $cached_info['latest_add_on_version'];
		}
		$stored_license_data['new_version'] = version_compare( Echo_Widgets::$version, $latest_add_on_version, "<" ) == 0 ? null : $latest_add_on_version;
		...
		if ( ! empty($license_error) && ! in_array($license_error, array('site_inactive', 'expired', 'no_activations_left', 'disabled', 'license_not_activable')) ) {
			...
			$expiry_msg = widg_get_instance()->license_handler->get_expiry_warning( $expiry_date );
		}
		...
		if ( widg_get_instance()->is_dormant ) {
		...
		WIDG_Utilities::save_wp_option('widg_last_license_check', $stored_license_data, true);

		// store last known status
		WIDG_Utilities::save_wp_option( WIDG_License_Handler::WIDG_LICENSE_STATE, array( $status_msg, $license_state, $latest_add_on_version, $expiry_date ), true );
