...
class appConfig
{
    ...
    { ?>
        <div style="width: 68%">
            <div class="mo-ms-tab-content-left-border">
                ...
                if (!empty($config) && "success" === get_option("mo_sps_test_connection_status")) {
    ...
    ?>
        <div class="mo-ms-tab-content-tile" style="width:135%;">
            <div class="mo-ms-tab-content-tile-content">
            	<div style="display: flex; align-items: center;height: 35px !important;">
                   <p style="font-size: 18px;font-weight: 650;display: inline-block"> Automatic Connection &nbsp;&nbsp;</p>
                   <a class="mo_sps_newbanner_manage-apps-button" target="_blank" style="text-decoration: none" href="<?php echo esc_url(pluginConstants::AUTO_CONNECTION_SETUP); ?>"><span class="dashicons dashicons-book-alt" style="font-size: large;font-weight:800;padding-right: 5px"></span>Setup Guide</a>
                </div>
                <div id="basic_attr_access_desc" class="mo_sps_help_desc" style="margin-bottom:20px;font-weight:500;">
                    <span>Effortlessly integrate your WordPress site with SharePoint and OneDrive sites using our pre-integrated application</span>
                </div>
                    ...
                    <span class="mo_sps_auto_connection" id="mo_sps_auto_connection_span" style="position:relative">
                        ...
                        <div style="font-size: 15px;position: relative;" id="mo_sps_auto_connection_select_container">
                            <button class="mo_sps_auto_connection_select" id="mo_sps_auto_connection_select" data-test="dropdown-trigger" type="button">
                                <img id="mo_sps_auto_connection_arrow_down" class="mo_sps_auto_connection_arrow_down" src="<?php echo esc_url(MO_SPS_PLUGIN_URL . '/images/down-arrow.svg'); ?>">
                                <img id="mo_sps_auto_connection_arrow_up" class="mo_sps_auto_connection_arrow_up" src="<?php echo esc_url(MO_SPS_PLUGIN_URL . '/images/down-arrow.svg'); ?>">
                            </button>
                        </div>
                        <div class="mo_sps_auto_connection_select_drpdn" id="mo_sps_auto_connection_select_drpdn">
                            <ul class="mo_sps_auto_connection_select_ul">
                                <li id="mo_sps_auto_connection_type_sharepoint" data-type="sharepoint" class="mo_sps_auto_connection_select_li <?php echo ($selected_connector == 'sharepoint' ? 'mo_sps_auto_connection_selected_li' : ''); ?>"><img width="14px" height="14px" src="<?php echo esc_url(MO_SPS_PLUGIN_URL . '/images/microsoft-sharepoint.svg'); ?>">SharePoint</li>
                                <li id="mo_sps_auto_connection_type_onedrive" data-type="onedrive" class="mo_sps_auto_connection_select_li <?php echo ($selected_connector == 'onedrive' ? 'mo_sps_auto_connection_selected_li' : ''); ?>"><img width="14px" height="14px" src="<?php echo esc_url(MO_SPS_PLUGIN_URL . '/images/onedrive.png'); ?>">OneDrive Business</li>
                                <li id="mo_sps_auto_connection_type_personal" data-type="personal" class="mo_sps_auto_connection_select_li <?php echo ($selected_connector == 'personal' ? 'mo_sps_auto_connection_selected_li' : ''); ?>"><img width="14px" height="14px" src="<?php echo esc_url(MO_SPS_PLUGIN_URL . '/images/onedrive.png'); ?>">OneDrive Personal</li>
                            </ul>
                        </div>
                    </span>
                </div>
    ...
    ?>
        ...
        <div class="mo-ms-tab-content-tile" style="width:135%;">
            <div class="mo-ms-tab-content-tile-content">
                <div style="display: flex; align-items: center;height: 35px !important;">
                   <p style="font-size: 18px;font-weight: 650;display: inline-block"> Manual Configuration &nbsp;&nbsp;</p>
                   <a class="mo_sps_newbanner_manage-apps-button" target="_blank" style="text-decoration: none" href="<?php echo esc_url(pluginConstants::MANUAL_CONNECTION_SETUP); ?>"><span class="dashicons dashicons-book-alt" style="font-size: large;font-weight:800;padding-right: 5px"></span>Setup Guide</a>
                </div>
                <div id="basic_attr_access_desc" class="mo_sps_help_desc" style="margin-bottom:20px;font-weight:500;">
                    <span>To begin document synchronization from SharePoint, create an Azure AD application to ensure secure communication with SharePoint content
                    </span>
                </div>
    ...
    private function mo_sps_display_user_info($response)
    {
        ...
        $user_details = [];
        $connector_image = esc_url(MO_SPS_PLUGIN_URL . '/images/microsoft-sharepoint.svg');

        if($app_type == 'auto') {
            $is_client_secret_expired = wpWrapper::mo_sps_check_client_secret_expiry_customers();
            $user_details['user_detail'] = 'Email ID :';
            $user_details['connection_type'] = 'Automatic';
            $user_details['connection_status'] = $is_client_secret_expired ? 'Inactive' : 'Active';
            switch($connector) {
                case 'personal': $response = isset($response[0]) && !empty($response[0]) ? $response[0] : $response;
                    ...
                    $user_details['user_email'] = isset($config['upn']) ? $config['upn'] : '';
                    $quota = isset($response['quota']) ? $response['quota'] : [];
                    $storage = $this->mo_sps_get_storage($quota);
                    $connector_image = esc_url(MO_SPS_PLUGIN_URL . '/images/onedrive.png');
                    break;
                case 'onedrive': $response = isset($response[0]) && !empty($response[0]) ? $response[0] : $response;
                    $owner = isset($response['owner']) ? $response['owner'] : [];
                    $user = isset($owner['user']) ? $owner['user'] : [];
                    $user_details['user_name'] = isset($user['displayName']) ? $user['displayName'] : "";
                ...
                case 'sharepoint': 
                    ...
                    if(!isset($config['email']) || !isset($config['name'])) {
                        ...
                        if($user['status']) {
                            ...
                            $config['email'] = isset($user['data']['mail']) ? $user['data']['mail'] : $config['upn'];
                        }
                    ...
                    $quota = wpWrapper::mo_sps_get_option(pluginConstants::SPS_QUOTA);
            ...
            <div class="mo-ms-tab-content-tile" style="width:138%; padding: 10px; border: 1px solid #ddd;">
                <div class="mo-ms-tab-content-tile-content">
                    <div style="display:flex;padding: 0px 0px 15px 5px;margin-left: -45px;">
                        ...
                        <img style="width:50px;height:50px;" src="<?php echo $connector_image; ?>">
                            <div class="mo-ms-display-user-info-div1-innerdiv">
                                <div class="<?php echo ('Active' === $user_details['connection_status'] ? 'circle' : 'circle_red'); ?>"></div>
                        ...
                        <div class="mo-ms-display-user-info-div3">
                            ...
                            <div style="display:flex;flex-direction:row;">
                                <div><button class="mo_copy copytooltip rounded-circle float-end" style="border:none;background:transparent;cursor:pointer;" type="submit" id="RemoveAccountButton" value="Remove Account"><span class="dashicons dashicons-trash"></span><span style="margin-left:5px;" class="copytooltiptext">Delete</span></button></div>
                            </div>
                        </div>
                    </div>
                </div>
...
<?php
    ...
    private function mo_sps_get_storage($quota){
        if(!empty($quota)){
