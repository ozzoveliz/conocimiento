...
class Azure {
	...
	private function setEndpoints() {
		...
		$this->endpoints['file_info']              = 'https://' . $api_endpoint . '/v1.0/drives/%s/items/%s';
		...
		$this->endpoints['search_personal_drive']  = "https://api.onedrive.com/v1.0/drives/%s/view.search?q=%s&\$select=name,size,webUrl,@content.downloadUrl,lastModifiedDateTime,id,folder,file&\$filter=contains(name, '%s')";
		$this->endpoints['search_personal_folder'] = "https://api.onedrive.com/v1.0/drives/%s/items/%s/view.search?q=%s&\$select=name,size,webUrl,@content.downloadUrl,lastModifiedDateTime,id,folder,file&\$filter=contains(name, '%s')";
	...
	public function mo_sps_process_tokens_for_auto_connection() {
		...
		if ( $response['status'] ) {
			...
			if ( 'personal' === $this->config['connector'] ) {
				...
				$jwt_object         = json_decode( base64_decode( str_replace( '_', '/', str_replace( '-', '+', $access_token_1 ) ) ), true );
	...
	/**
	 * Function to call search api and return the api response.
	 *
	 * @param [int]    $drive_id
	 * @param [string] $query_text
	 * @return void
	 */
	public function mo_sps_search_through_drive_items( $drive_id, $query_text, $connector ) {
		$access_token = $this->mo_sps_send_access_token();
		...
		if ( ( 'personal' === $connector ) ) {
			$response = $this->handler->mo_sps_get_request( sprintf( $this->endpoints['search_personal_drive'], $drive_id, $query_text, $query_text ), $args );
		} else {
	 ...
	 * @return void
	 */
	public function mo_sps_search_through_folder_items( $drive_id, $folder_id, $query_text, $connector ) {
		$access_token = $this->mo_sps_send_access_token();
		...
		if ( ( 'personal' === $connector ) ) {
			$response = $this->handler->mo_sps_get_request( sprintf( $this->endpoints['search_personal_folder'], $drive_id, $folder_id, $query_text, $query_text ), $args );
		} else {
			$response = $this->handler->mo_sps_get_request( sprintf( $this->endpoints['search_folderitems'], $drive_id, $folder_id, $query_text ), $args );
		}
