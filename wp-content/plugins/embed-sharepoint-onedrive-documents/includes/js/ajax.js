jQuery(document).ready(function ($) {
  ...
  if(config['is_plugin'] == 'n') {
    ...
    config['drive_id'] = doc_sync_data.selected_drive??'';
    config['folder_id'] = doc_sync_data.selected_folder??'';
    config['folder_path'] = doc_sync_data.selected_folder_path??'';
    config['drive_name'] = doc_sync_data.selected_drive_name??'';
  ...
  breadcrumbs = $.isEmptyObject(saved_breadcrumb) || saved_breadcrumb.length <= 1 ? [`<span index="0" class="mo_sps_doc_breadcrumbs_items" data-id="${config['drive_id']}" id="mo_sps_breadcrumb_drive">${config['drive_name']}</span>`] : saved_breadcrumb;
  ...
  function process_docs(res,searched = false) {
    let docs = res.data.value;
    if((config['currentView'] === 'folder' || config['currentView'] === 'folder_path') && docs && !$.isEmptyObject(docs)) {
      var item = docs[0];
      if(item.parentReference && item.parentReference.id) {
        config['folder_id'] = item.parentReference.id;
      }
    }
    ...
    if($.isEmptyObject(docs)) {
      ...
      docs.forEach(doc=>{
        if('folder' in doc) {
          content += `<tr class="mo_sps_table_tr" style="cursor:pointer;" id="mo_sps_folder_doc_sync__${doc.id}" item_name="${doc.name}" web-url="${doc.webUrl}" item_path="${doc.parentReference && doc.parentReference.path ? doc.parentReference.path : ``}" item_id="${doc.id}">
            <td class="mo_sps_table_tbody_td" style="display:flex;"><img style="width:20px;height:20px;margin-right:10px;" src="${doc_sync_data.folder_icon_url}" >
              <div style="max-width: 18rem;overflow-x: hidden;text-overflow: ellipsis;word-wrap: normal !important;white-space: nowrap !important;">${doc.name}</div>
                ...
                <span class="mo_sps_share_download_span copytooltip-span">
                  <img web-url="${doc.webUrl}" data-id="${doc.id}" id="mo_sps_folder_doc_preview_${doc.id}" style="width:20px;height:20px;cursor:pointer;" src="${doc_sync_data.redirect}">
                  <span class="copytooltiptext-span">Preview in Sharepoint</span>
                </span>
              </div>
            </td>
            <td class="mo_sps_table_tbody_td">${mo_sps_formatDate(doc.lastModifiedDateTime.split('T')[0])}</td>
          ...
          var download_url = doc['@microsoft.graph.downloadUrl'] ?? doc['@content.downloadUrl'] ?? '';
          files_download_url.push(download_url);
          ...
          content += `<tr class="mo_sps_table_tr">
            <td class="mo_sps_table_tbody_td" style="display:flex;"><img style="width:20px;height:20px;margin-right:10px;" src="${file_url}">
              ${download_url || config['searched_doc'] ? `<div id="mo_sps_file_doc_sync__${doc.id}" key="${files_download_url.length - 1}" searched="${searched}" file-id="${doc.id}" class="mo_sps_table_tr_file_name_div" style="max-width: 16rem;overflow-x: hidden;text-overflow: ellipsis;white-space:nowrap !important;">${doc.name}</div>` : `<div searched="${searched}" style="max-width: 16rem;overflow-x: hidden;text-overflow: ellipsis;white-space:nowrap !important;">${doc.name}</div>`}
              <div id="mo_sps_file_share_download" class="mo_sps_file_share_download">
                ${download_url || config['searched_doc'] ? `<span class="mo_sps_share_download_span copytooltip-span">
                  <img file-id="${doc.id}" drive-id="${config['drive_id']}" id="mo_sps_file_doc_download_${doc.id}" searched="${searched}" key="${files_download_url.length - 1}" style="width:20px;height:20px;cursor:pointer;" src="${doc_sync_data.download}">
                  <span class="copytooltiptext-span">Download</span>
                </span>`: ``}
                <span class="mo_sps_share_download_span copytooltip-span">
                  <img web-url="${doc.webUrl}" data-id="${doc.id}" id="mo_sps_file_doc_preview_${doc.id}" style="width:20px;height:20px;cursor:pointer;" src="${doc_sync_data.redirect}">
                  <span class="copytooltiptext-span">Preview in Sharepoint</span>
                </span>
              </div>
            </td>
            <td class="mo_sps_table_tbody_td">${mo_sps_formatDate((doc.lastModifiedDateTime).split('T')[0])}</td>
            <td class="mo_sps_table_tbody_td">${doc.size ? mo_sps_formatBytes(doc.size, 0) : 0}</td>
          </tr>`
        }
      });
    }
  ...
  function process_search_documents(res) {
    ...
    all_docs.forEach(doc=>{
      if('folder' in doc) {
        content += `<div id="mo_sps_search_drpdn_folder_${doc.id}" item_name="${doc.name}" item_id="${doc.id}" web-url="${doc.webUrl}" class="list_items"><div><img style="display:block;width:1.2rem;height:1.2rem;margin-right:10px;" src="${doc_sync_data.folder_icon_url}" alt></div><div><div style="width:11rem"><span style="word-wrap:break-word;font-size:0.9rem;">${doc.name}</span></div></div></div>`;
      } else {
        let searched = true;
        let file_name_crump = (doc.name).split('.');
        let file_type = file_name_crump[file_name_crump.length - 1];
        ...
        if (content_type && content_type.includes("image"))
          ...
          file_url = doc_sync_data.pdfdoc_icon;
        content += `<div id="mo_sps_search_drpdn_file_${doc.id}" searched="${searched}" file-id="${doc.id}" class="list_items"><div><img style="display:block;width:1.2rem;height:1.2rem;margin-right:10px;" src="${file_url}" alt></div><div><div style="width:11rem"><span style="word-wrap:break-word;font-size:0.9rem;">${doc.name}</span></div></div></div>`;
      }
    });
  ...
  function handle_events() {
    ...
    $(document).on('click', "img[id^='mo_sps_file_doc_download_'],div[id^='mo_sps_file_doc_sync__'],div[id^='mo_sps_search_drpdn_file_']", function(e){
      ...
      var is_searched = ele.getAttribute('searched');
      ...
      else
      {
        ...
        download_url = files_download_url[key] ?? files_download_url[key];
        window.location.href = download_url;
      }

    });

    $(document).on('click', "img[id^='mo_sps_file_doc_preview_']", function(e){
      e.preventDefault();
      e.stopPropagation();
      var ele = e.currentTarget;
      ...
      if(config['searched_doc'] === true) {
        var ajax_url = `${doc_sync_data.admin_ajax_url}?action=mo_sps_get_file_web_url&drive_id=${config['drive_id']}&file_id=${file_id}`;
    ...
    $(document).on('click', "img[id^='mo_sps_folder_doc_preview_']", function(e){
      e.preventDefault();
      e.stopPropagation();
      var ele = e.currentTarget;
      var web_url = ele.getAttribute('web-url');
      ...
      // var relative_path = web_url.split(encodeURI(config['drive_name'])).pop();
        ...
        if(relative_path) {
          ...
          for(var i = 1; i<path_ele.length; i++) {
            ...
            breadcrumbs.push(`<span index="${breadcrumbs.length}" class="mo_sps_doc_breadcrumbs_items" data-path="${item_folder_path}" data-id="${config['folder_id']}" data-name="${folder_name}" id="mo_sps_breadcrumb_folder_${config['folder_id']}">${folder_name}</span>`)
    ...
    $(document).on('click', `#mo_sps_doc_refresh`, function(e) {
      load_breadcrumbs(breadcrumbs);
      load_paths[config['currentView']].init();
    });
  }
  ...
  function mo_sps_show_error(res) {
    res = res.data && res.data.data ? res.data.data : 'error';
    $('#mo_sps_doc_table').hide();
    $(`#mo_sps_drive_not_selected`).hide();
    $('#mo_sps_site_drive_not_selected').hide();
    ...
    var content = `<div id="mo_sps_doc_embed_error" class="mo_sps_table_loader_div">
        ...
        &nbsp;
        <div style="text-align:center;">${res.error_description ? res.error_description : 'Something went wrong.'}</div>
    </div>`;
