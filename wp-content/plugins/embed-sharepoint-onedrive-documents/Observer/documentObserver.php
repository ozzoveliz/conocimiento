...
class documentObserver {
	...
	private function mo_sps_document_search_observer( $payload ) {
		...
		if ( ! empty( $connector ) ) {
			...
			if ( '' === $folder_id ) {
					$response = $client->mo_sps_search_through_drive_items( $drive_id, $query_text, $connector );
			} else {
				$response = $client->mo_sps_search_through_folder_items( $drive_id, $folder_id, $query_text, $connector );
			}
			if ( ! empty( $response['status'] ) ) {
				wp_send_json_success( $response['data'] );
			} else {
	...
	public function mo_sps_get_file_web_url() {
		$config      = wpWrapper::mo_sps_get_option( pluginConstants::APP_CONFIG );
		$api_handler = Azure::getClient( $config );
