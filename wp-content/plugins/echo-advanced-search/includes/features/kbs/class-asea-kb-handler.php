 ...
 */
class ASEA_KB_Handler {

	// Prefix for custom post type name associated with given KB; this will never change
	const KB_POST_TYPE_PREFIX = ASEA_KB_Core::ASEA_KB_POST_TYPE_PREFIX;  // changing this requires db update
	...
	public static function get_post_type( $kb_id ) {
		$kb_id = ASEA_Utilities::sanitize_int($kb_id, ASEA_KB_Core::DEFAULT_KB_ID );
	...
	public static function get_first_kb_main_page_url( $kb_config ) {
		...
		foreach ( $kb_main_pages as $post_id => $post_title ) {
			...
			if ( ASEA_Core_Utilities::is_wpml_enabled_addon( $kb_config['id'] ) ) {
