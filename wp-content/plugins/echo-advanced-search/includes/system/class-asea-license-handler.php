...
class ASEA_License_Handler {
	...
	const PLUGIN_NAME           = 'Advanced Search';
	const LICENSE_KEY_OPTION_ID = 'asea_license_key';
	const ASEA_LICENSE_STATE    = 'asea_license_state';
	...
	function __construct( $_file, $_version ) {
		...
		$this->license_key = ASEA_Utilities::get_wp_option( self::LICENSE_KEY_OPTION_ID, '' );
		$this->setup_hooks();
	}
	...
	private function setup_hooks() {
		...
		add_filter( ASEA_KB_Core::ASEA_KB_ADD_ON_LICENSE_MSG, array( $this, 'get_license_status_short') );
	}
	...
	public function auto_updater() {
		...
		// set up the updater
		if ( ! class_exists( 'ASEA_EDD_SL_Plugin_Updater' ) ) {
			require_once 'class-asea-edd-sl-plugin-updater.php';
		}

		new ASEA_EDD_SL_Plugin_Updater(
	...
	public function get_cached_info() {
		$default_array = array( __( 'Unknown status.', 'echo-knowledge-base' ), '', Echo_Advanced_Search::$version, '' );
		$asea_license_state = ASEA_Utilities::get_wp_option( self::ASEA_LICENSE_STATE, $default_array, true );
		$status_msg = isset($asea_license_state[0]) ? $asea_license_state[0] : __( 'Unknown status.', 'echo-knowledge-base' );
		$license_state = isset($asea_license_state[1]) ? $asea_license_state[1] : '';
		$latest_add_on_version = isset($asea_license_state[2]) ? $asea_license_state[2] : Echo_Advanced_Search::$version;
		$expiry_date = isset($asea_license_state[3]) ? $asea_license_state[3] : '';
	...
	public function handle_license_command() {
		...
		if ( ! isset( $_REQUEST['_wpnonce_asea_license_key'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce_asea_license_key'], '_wpnonce_asea_license_key' ) ) {
			ASEA_Utilities::ajax_show_error_die( __( 'Security check failed.', 'echo-knowledge-base' ) );
		}

		// need license key to activate/deactivate the license
		$entered_license_key = empty($_REQUEST['asea_license_key']) ? '' : sanitize_text_field( $_REQUEST['asea_license_key'] );
	...
	private function save_license( $license_key ) {
		...
		$result = ASEA_Utilities::save_wp_option( self::LICENSE_KEY_OPTION_ID, $license_key, true );
		if ( is_wp_error( $result ) ) {
			/* @var $result WP_Error */
			$message = $result->get_error_data();
			if ( empty($message) ) {
				ASEA_Utilities::ajax_show_error_die( $result->get_error_message(),  __( 'Could not save settings.', 'echo-knowledge-base' ) );
			}
			ASEA_Utilities::ajax_show_error_die( $this->generate_error_summary( $result->get_error_data() ), __( 'Settings NOT saved due to following problems:', 'echo-knowledge-base' ) );
		}
	}
	...
	public function contact_license_server( $action, $license_key ) {
		...
		if ( is_wp_error( $response ) ) {
			$error_msg = '<br/>REQUEST: url => ' . $get_url . ', args => ' . ASEA_Utilities::get_variable_string( $get_args );
			$error_msg .= '<br/>RESPONSE: ' . ASEA_Utilities::get_variable_string( $response );
	...
	public function get_expiry_warning( $expiry_date ) {
		...
		$difference_in_days = ASEA_Utilities::get_days_since( '', $expiry_date );
	...
	public function get_license_state( $force_refresh=false ) {
		...
		$stored_license_data = ASEA_Utilities::get_wp_option('asea_last_license_check', array(), true, true);
		if ( is_wp_error($stored_license_data) ) {
			ASEA_Logging::add_log("Could not retrieve license data option.");
			return null;
		}
		...
		$licenses_tab_url = admin_url( 'edit.php?post_type=' . ASEA_KB_Handler::get_post_type( ASEA_KB_Core::DEFAULT_KB_ID ) . '&page=ep' . 'kb-add-ons&ep' . 'kb-tab=licenses' );
		...
		if ( ! empty($license_error) && $this->is_connection_error( $license_error ) ) {
			...
			if ( ! empty($stored_license_data['last_connection_error']) && ASEA_Utilities::get_days_since( $stored_license_data['last_connection_error'] ) > 30 ) {
		...
		$stored_license_data['new_version'] = version_compare( Echo_Advanced_Search::$version, $latest_add_on_version, "<" ) == 0 ? null : $latest_add_on_version;
		...
		ASEA_Utilities::save_wp_option('asea_last_license_check', $stored_license_data, true);
