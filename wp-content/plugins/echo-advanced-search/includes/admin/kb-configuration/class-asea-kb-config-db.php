 ...
 */
class ASEA_KB_Config_DB {

	// Prefix for WP option name that stores KB configuration
	const KB_CONFIG_PREFIX =  'asea_config_';
	...
	function get_kb_configs( $skip_check=false ) {
		...
		if ( ! empty($this->cached_settings) && $this->is_cached_all_kbs ) {
			...
			foreach( $this->cached_settings as $config ) {
				...
				$kb_id = $config['id'];
				$kb_options_checked[$kb_id] = wp_parse_args( $config, ASEA_KB_Config_Specs::get_default_kb_config() );
			}
		...
		if ( empty($kb_options) || ! is_array($kb_options) ) {
			ASEA_Logging::add_log("Did not retrieve any kb config. Using defaults (22). Last error: " . $wpdb->last_error, $kb_options);
			$kb_options = array();
		}
		...
		foreach ( $kb_options as $ix => $row ) {
			...
			if ( $config === false ) {
				ASEA_Logging::add_log("Could not unserialize configuration: ", ASEA_Utilities::get_variable_string($row['option_value']));
				continue;
			}

			if ( empty($config) || ! is_array($config) ) {
				ASEA_Logging::add_log("Did not find configuration");
				continue;
			}

			if ( empty($config['id']) ) {
				ASEA_Logging::add_log("Found invalid configuration", $config);
				continue;
			}

			$kb_id = ( $config['id'] === self::DEFAULT_KB_ID ) ? $config['id'] : ASEA_Utilities::sanitize_get_id( $config['id'] );
			...
			if ( ASEA_Core_Utilities::is_wpml_enabled_addon( $kb_id ) ) {
				...
				if ( empty($config) || ! is_array($config) ) {
					ASEA_Logging::add_log( "Did not find KB configuration, the default was used instead.", $kb_id );
					$config = array();
				}
			}
			...
			$kb_options_checked[$kb_id] = ASEA_Core_Utilities::update_from_editor_config( $kb_options_checked[$kb_id] );
		...
		if ( empty($kb_options_checked) || ! isset($kb_options_checked[self::DEFAULT_KB_ID]) ) {
			ASEA_Logging::add_log("Need at least default configuration.");
			$kb_options_checked[self::DEFAULT_KB_ID] = ASEA_KB_Config_Specs::get_default_kb_config();
		}
	...
	public function get_kb_ids( $ignore_error=false ) {
		...
		$kb_option_names = $wpdb->get_results("SELECT option_name FROM $wpdb->options WHERE option_name LIKE '" . ASEA_KB_Core::ASEA_KB_CONFIG_PREFIX . "%'", ARRAY_A );
		if ( empty($kb_option_names) || ! is_array($kb_option_names) ) {
			if ( ! $ignore_error ) {
				ASEA_Logging::add_log("Did not retrieve any kb config. Try to deactivate and active KB plugin to see if the issue will be fixed (11). Last error: " . $wpdb->last_error, $kb_option_names);
			}
		...
		foreach ( $kb_option_names as $kb_option_name ) {
			...
			$kb_id = str_replace( ASEA_KB_Core::ASEA_KB_CONFIG_PREFIX, '', $kb_option_name['option_name'] );
			$kb_id = ASEA_Utilities::sanitize_int( $kb_id, self::DEFAULT_KB_ID );
			$kb_ids[$kb_id] = $kb_id;
		}
	...
	public function get_kb_config( $kb_id, $return_error=false ) {
		...
		$kb_id = ( $kb_id === self::DEFAULT_KB_ID ) ? $kb_id : ASEA_Utilities::sanitize_get_id( $kb_id );
		if ( is_wp_error($kb_id) ) {
			return $return_error ? $kb_id : ASEA_KB_Config_Specs::get_default_kb_config();
		}
		/** @var int $kb_id */

		// retrieve settings if already cached
		if ( ! empty($this->cached_settings[$kb_id]) ) {
			$config = wp_parse_args( $this->cached_settings[$kb_id], ASEA_KB_Config_Specs::get_default_kb_config() );
			$config['id'] = $kb_id;
			// filter kb config for Editor 
			return ASEA_Core_Utilities::update_from_editor_config( $config );
		}
		...
		if ( empty($config) || ! is_array($config) ) {
			// if new KB then this will happen: ASEA_Logging::add_log("Did not find KB configuration (DB231).", $kb_id);
			return $return_error ? new WP_Error('DB231', "Did not find KB configuration. Try to deactivate and reactivate KB plugin to see if this fixes the issue. " . ASEA_Utilities::contact_us_for_support() )
								 : ASEA_KB_Config_Specs::get_default_kb_config();
		}

		// use defaults for missing or empty fields
		$config = wp_parse_args( $config, ASEA_KB_Config_Specs::get_default_kb_config() );
		$config['id'] = $kb_id;

		// filter kb config for Editor
		$config = ASEA_Core_Utilities::update_from_editor_config( $config );
	...
	public function get_kb_config_or_default( $kb_id ) {
		...
		if ( empty( $kb_config ) || ! is_array( $kb_config) || is_wp_error( $kb_config ) ) {
			return ASEA_KB_Config_Specs::get_default_kb_config();
		}
	...
	public function get_value( $kb_id, $setting_name, $default = '' ) {
		...
		$kb_config = empty($kb_id) ? ASEA_KB_Config_Specs::get_default_kb_config() : $this->get_kb_config( $kb_id );

		if ( isset($kb_config[$setting_name]) ) {
			return $kb_config[$setting_name];
		}

		$default_settings = ASEA_KB_Config_Specs::get_default_kb_config();
	...
	public function update_kb_configuration( $kb_id, array $config, $upsert=true ) {
		...
		$fields_specification = ASEA_KB_Config_Specs::get_fields_specification();
		$input_filter = new ASEA_Input_Filter();
		$sanitized_config = $input_filter->validate_and_sanitize_specs( $config, $fields_specification );
		if ( is_wp_error($sanitized_config) ) {
			ASEA_Logging::add_log( 'Could not update the configuration', $kb_id, $sanitized_config );
			return $sanitized_config;
		}

		// use defaults for missing configuration
		$sanitized_config = wp_parse_args( $sanitized_config, ASEA_KB_Config_Specs::get_default_kb_config() );
	...
	private function save_kb_config( array $config, $kb_id, $upsert=true ) {
		...
		// KB configuration always starts with asea_config_[ID]
		...
		if ( $result === false ) {
			ASEA_Logging::add_log( 'Failed to update kb config for kb_id', $kb_id, 'Last DB ERROR: (' . $wpdb->last_error . ')' );
	...
	public function add_new_add_on_config( $kb_id ) {
		$this->update_kb_configuration( $kb_id, ASEA_KB_Config_Specs::get_default_kb_config() );
	}
	
	/**
	 * Filter KB config to add ASEA values 
	 * @param $kb_config
	 * @return array
	 */
