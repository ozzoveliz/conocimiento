...
function epkb_main_content( $args ) {
	...
	global $wp_query;
	$initial_posts_per_page = empty( $wp_query->query_vars['posts_per_page'] ) ? (int)get_option( 'posts_per_page' ) : $wp_query->query_vars['posts_per_page'];
	$posts_counter = $initial_posts_per_page;
	$initial_paged = get_query_var( 'paged' ) ? get_query_var( 'paged' ) : 1;
	$skip_posts = ($initial_paged - 1) * $initial_posts_per_page;
	...
	<main class="eckb-category-archive-main <?php esc_attr_e($preset_style); ?>">   <?php
		...
		while ( have_posts() ) {
			...
			the_post();
			/** <========== AMGR specific code - START */
			// skip posts that are not for our paginate
			if ( $skip_posts ) {
				$skip_posts--;
				continue;
			}

			if ( empty( $posts_counter ) ) {
				continue;
			}
		...
		/** <========== AMGR specific code - START */
		// return values as should be to show right pagination
		$wp_query->max_num_pages = ceil( $wp_query->found_posts / $initial_posts_per_page);
		$wp_query->set( 'paged', $initial_paged );
		/** <========== AMGR specific code - START */
