...
class EPKB_Config_Tools_Page {
	...
	public static function get_tools_view_config( $kb_config ) {
		...
		$secondary_tabs[] = AMGR_WP_Roles::use_kb_groups() ? null : array(		// KB Groups can have access to a specific KB only; this complicates access and ordering
	...
	private static function get_debug_boxes( $kb_config ) {
		...
		return array(
			...
			/** AMGR ONLY **/
			// Box: Debug areas, restricted for users
			array(
				'title' => __( 'Debug User Access', 'echo-knowledge-base' ),
				'description' => __( 'Enable debug when instructed by the support team.', 'echo-knowledge-base' ),
	...
	private static function display_debug_info( $kb_config ) {
		...
		<div id="epkb_debug_info_tab_page">
			...
			if ( ! empty( $is_debug_on ) ) { ?>
				...
				echo self::display_amgr_debug_data();   /**   AMGR   **/  ?>
	...
	public static function display_amgr_debug_data() {
		/** @var $wpdb Wpdb */
		global $wpdb;
		...
		if ( is_wp_error( $result ) ) {
			...
			foreach ( $result as $row ) { ?>
				<tr>
				<td><?php echo $row->name; ?></td> <td><?php echo $row->kb_group_id; ?></td> <td><?php echo $row->kb_id; ?></td>
				</tr><?php
			} ?>
			</table><?php
	...
	private static function display_debug_errors() {
		...
		$logs += AMGR_Logging::get_logs();   /**   AMGR   **/
