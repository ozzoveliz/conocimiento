...
class EPKB_Admin_UI_Access {
	...
	const EPKB_KB_CONTRIBUTOR_CAPABILITY = 'edit_xxxs';	// AMGR

	// Author capability
	const EPKB_WP_AUTHOR_CAPABILITY = 'publish_posts';
	const EPKB_KB_AUTHOR_CAPABILITY = 'publish_xxxs';	// AMGR

	// Editor capability
	const EPKB_WP_EDITOR_CAPABILITY = 'manage_categories';
	const EPKB_KB_EDITOR_CAPABILITY = 'manage_xxx_categories';	// AMGR

	// Admin capability
	const EPKB_ADMIN_CAPABILITY = 'manage_options';
	const EPKB_KB_MANAGER_CAPABILITY = 'admin_eckb_access_manager_page';	// AMGR

	// KB Editor
	const EPKB_KB_MANAGER_CONFIG = 'admin_eckb_access_config_write';	// AMGR
	...
	const ADMIN_UI_CONTEXTS = array(
		...
		// AMGR: additionally, KB Managers can access contexts the following Admin contexts
	...
	public static function is_user_access_to_context_allowed( $context ) {
		...
		// AMGR: KB manager can access any KB context
	...
	public static function get_context_required_capability( $contexts, $kb_config=null ) {
		...
		// AMGR: CAPABILITY LEVEL 3: check if any context is in allowed admin UI contexts for 'KB Manager'
		foreach ( $contexts as $context ) {

			if ( in_array( $context, self::ADMIN_UI_CONTEXTS ) ) {
	...
	private static function is_context_allowed( $kb_id, $capability, $specs, $context ) {
		...
		foreach ( $specs[$context]['allowed_access'] as $ix => $allowed_access ) {
			...
			// AMGR: KB Groups does not allow KB Author and KB Editor to use Editor or Ordering of restricted Articles / Categories
	...
	public static function get_contributor_capability( $kb_id=0 ) {
		...
		if ( AMGR_WP_Roles::use_kb_groups() ) {
			...
			return str_replace( 'xxx', $capability_type, self::EPKB_KB_CONTRIBUTOR_CAPABILITY );
		}
	...
	public static function get_author_capability( $kb_id=0 ) {
		...
		if ( AMGR_WP_Roles::use_kb_groups() ) {
			$kb_id = empty( $kb_id ) ? EPKB_KB_Handler::get_relevant_kb_id() : $kb_id;
			$capability_type = AMGR_Access_Utilities::get_capability_type( $kb_id );
			return str_replace( 'xxx', $capability_type, self::EPKB_KB_AUTHOR_CAPABILITY );
		}
	...
	public static function get_editor_capability( $kb_id=0 ) {
		...
		if ( AMGR_WP_Roles::use_kb_groups() ) {
			$kb_id = empty( $kb_id ) ? EPKB_KB_Handler::get_relevant_kb_id() : $kb_id;
			$capability_type = AMGR_Access_Utilities::get_capability_type( $kb_id );
			return str_replace( 'xxx', $capability_type, self::EPKB_KB_EDITOR_CAPABILITY );
		}
		...
		$boxes_config[] =
			array(
				...
				'html' => self::radio_buttons_vertical_access_control( array(
					...
					'options'       => self::get_access_control_options( $kb_config ) ) ) );
		...
		$boxes_config[] =
			array(
				...
				'html' => self::radio_buttons_vertical_access_control( array(
					...
					'options'       => self::get_access_control_options( $kb_config, true ) ) ) );
	 ...
	 * @return array
	 */
	private static function get_access_control_options( $kb_config, $include_author=false ) {
		...
		if ( AMGR_WP_Roles::use_kb_groups() ) {
			...
			if ( $include_author ) {
				$author_capability = str_replace( 'xxx', $capability_type, self::EPKB_KB_AUTHOR_CAPABILITY );
			...
			// Editor capability
			$editor_capability = str_replace( 'xxx', $capability_type, self::EPKB_KB_EDITOR_CAPABILITY );
	...
	public static function save_access_control() {
		...
		foreach ( self::ADMIN_UI_CONTEXTS as $context ) {
			...
			// AMGR: if KB Groups add-on is enabled then no need to look for contexts available for Admins and KB Managers only
			if ( AMGR_WP_Roles::use_kb_groups() && in_array( $context, self::AMGR_RESTRICTED_ADMIN_UI_CONTEXTS ) ) {
				continue;
			}
