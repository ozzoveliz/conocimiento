...
class EPKB_Layout_Article_Sidebar extends EPKB_Layout {
	 ...
	 *
	 * @param $kb_config
	 * @param bool $show_only_current_category - Category Archive Page has sidebar collapsed and only shows current category
	 */
	public function display_article_sidebar( $kb_config, $show_only_current_category=false, $current_category_id=null ) {
		...
		// AMGR protect KB Artilce Sidebar
		$kb_groups_set = AMGR_Access_Utilities::get_main_page_group_sets( $kb_config['id'], $this->category_seq_data, $this->articles_seq_data );
		if ( $kb_groups_set === null || ( empty( $kb_groups_set['categories_seq_data'] ) && empty( $kb_groups_set['articles_seq_data'] ) ) ) {
			$this->category_seq_data = [];
			$this->articles_seq_data = [];
		} else {
		...
		if ( $show_only_current_category ) {
			...
			if ( ! empty( $current_category_id ) ) {
				...
				// get parent term id if possible
				$sub_categories = EPKB_Categories_DB::get_sub_categories( $this->category_seq_data, $current_category_id  );
				if ( ! empty( $sub_categories ) ) {

					// remove array top elements that are not equal to the $current_category_id
					$sub_categories_filtered = array_filter( $sub_categories, function( $key ) use ( $current_category_id ) {
						return $key == $current_category_id;
					}, ARRAY_FILTER_USE_KEY );

					$this->category_seq_data = empty( $sub_categories_filtered ) ? $sub_categories : $sub_categories_filtered;
				}
			}
		}
		...
		if ( is_archive() ) {
			$this->kb_config['sidebar_top_categories_collapsed'] = 'on';
			$current_term = EPKB_Utilities::get_current_category();
			...
			if ( $article_num > $nof_articles_displayed ) { ?>
				<li class="epkb-show-all-articles" aria-expanded="false">
					<span class="epkb-show-text">
						<span><?php echo esc_html( $this->kb_config['sidebar_show_all_articles_msg'] ); ?></span><?php esc_html_e( $article_num - $nof_articles_displayed ); ?>
					</span>
