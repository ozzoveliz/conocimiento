...
class EPKB_Category_Archive_Setup {
	...
	public static function category_header( $args ) {
		...
		$term = EPKB_Utilities::get_current_category();
		if ( empty( $term ) ) {
			return;
		}
	...
	public static function archive_content_body( $args ) {
		...
		// AMGR protect Category Archive Page
		...
		<main class="eckb-category-archive-main">   <?php
			...
			if ( $total_articles > 0 ) {
				...
				<div class="eckb-article-list-container eckb-article-list-container-columns-<?php echo esc_attr( $nof_columns . ' ' . $article_list_separator_classes ); ?>"> <?php
					...
					self::display_category_articles( $kb_config, $term->term_id, $articles_list, $kb_config['archive_content_articles_nof_articles_displayed'] );
			...
			if ( $kb_config['archive_content_sub_categories_toggle'] == 'on' ) {
				...
				<div class="eckb-sub-category-list-container">  <?php
					self::show_sub_categories( $kb_config, $term->term_id, $category_seq_data, $articles_seq_data, $sub_categories ); ?>
				</div>  <?php
			}
	 ...
	 */
	private static function display_category_articles( $kb_config, $category_id, $articles_list, $nof_articles_displayed ) {
	...
	private static function show_sub_categories( $kb_config, $category_id, $category_seq_data, $articles_seq_data, $sub_categories ) {
				...
				foreach ( $sub_categories_in_row as $sub_category_id => $child_categories ) {
					...
					if ( $kb_config['archive_content_sub_categories_with_articles_toggle'] == 'on' ) {    ?>
						<div id="eckb-sub-category-id-<?php echo esc_attr( $sub_category_id ); ?>" class="eckb-sub-category-container">
							...
							<div class="eckb-sub-category-body"> <?php
								...
								if ( count( $articles_list ) > 0 ) { ?>
								    <div class="eckb-sub-category-article-list-container">   <?php
										self::display_category_articles( $kb_config, $sub_category_id, $articles_list, $kb_config['archive_content_sub_categories_nof_articles_displayed'] ); ?>
								    </div>  <?php
							...
							if ( $show_count > 0 ) { ?>
								<div class="eckb-sub-category-footer">
									<div class="eckb-sub-category-article-count">+ <?php echo esc_html( $show_count ) . ' ' . esc_html__( 'Articles', 'echo-knowledge-base' ) ?> </div>
									<a class="eckb-sub-category-show-all-link" href="<?php echo esc_url( $category_link ); ?>"><?php echo esc_html( $kb_config['show_all_articles_msg'] ); ?></a>
								</div>  <?php
							} ?>
