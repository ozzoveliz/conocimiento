...
class EPKB_Articles_CPT_Setup {
	...
	public function register_knowledge_base_post_types() {
		...
		$current_id = empty( $current_id ) ? EPKB_KB_Handler::get_first_kb_id() : $current_id;  // AMGR only
	...
	public static function register_custom_post_type( $kb_config, $current_id ) {
		...
		$args = [
				...
				'capabilities'       => self::get_cpt_capabilities( $capability_type ),
				'map_meta_cap'       => true,
	 ...
	 * @return array
	 */
	private static function get_cpt_capabilities( $capability_type ) {
		return array(
			// Post type
