 ...
 */
class AMGR_DB_KB_Public_Groups extends AMGR_DB  {
     ...
     * @var string
     */
    public $cache_group = 'amgr_kb_public_groups';
    ...
    public function __construct() {
        ...
        $this->table_name  = $wpdb->prefix . 'amgr_kb_public_groups';
	...
	public function get_public_group( $kb_id ) {
		...
		if ( empty($group) ) {
			AMGR_LOGGING::add_log( 'Could not find Public group.', $kb_id );
			return null;
		}
    ...
    public function insert_public_group( $kb_id ) {
	    ...
	    $kb_group_id = epkb_get_instance()->db_kb_groups->insert_group( $kb_id, self::PUBLIC_GROUP_NAME );
	    if ( empty($kb_group_id) ) {
		    AMGR_Logging::add_log('Could not create PUBLIC group.', $kb_id);
		    null;
	    }
	...
	/**
	 * Determine if given KB Groups is Public.
	 *
	 * @param $kb_id
	 * @param $kb_group_id
	 *
