 ...
 */
class AMGR_DB_Access_Read_Only_Categories extends AMGR_DB  {
     ...
     * @var string
     */
    public $cache_group = 'amgr_access_read_categories';
    ...
    public function __construct() {
        ...
        $this->table_name  = $wpdb->prefix . 'amgr_access_read_categories';
	...
	public function get_all_read_only_kb_category_ids( $kb_id ) {
		...
		if ( is_wp_error($kb_category_ids) ) {
			AMGR_Logging::add_log( "Cannot retrieve categories for given KB", $kb_id, $kb_category_ids );
			return null;
		}
	...
	public function add_read_only_group_to_category( $kb_id, $kb_group_id, $kb_category_id ) {
		...
		if ( empty($term) || ! $term instanceof WP_Term ) {
			AMGR_Logging::add_log("Category is not KB Category: " . $kb_category_id . " (31)", $kb_id);
			return false;
		}
