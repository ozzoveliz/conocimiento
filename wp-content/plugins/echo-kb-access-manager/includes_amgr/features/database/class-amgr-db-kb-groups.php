 ...
 */
class AMGR_DB_KB_Groups extends AMGR_DB  {
     ...
     * @var string
     */
    public $cache_group = 'amgr_kb_groups';
    ...
    public function __construct() {
        ...
        $this->table_name  = $wpdb->prefix . 'amgr_kb_groups';
	...
	public function get_group( $kb_id, $kb_group_id ) {
		...
		if ( is_wp_error( $group) ) {
			AMGR_Logging::add_log("Cannot get KB Group record: ", $kb_id);
			return null;
		}
	...
	public function get_private_groups( $kb_id ) {
		...
		foreach( $kb_groups as $kb_group ) {
			$is_group_public = epkb_get_instance()->db_kb_public_groups->is_public_group( $kb_id, $kb_group->kb_group_id );
			if ( ! $is_group_public ) {
    ...
    public function insert_group( $kb_id, $kb_group_name ) {
	    ...
	    if ( ! $this->validate_group_name( $kb_group_name) ) {
		    AMGR_Logging::add_log("KB Group name is not valid: ", $kb_group_name);
		    return null;
	    }
        ...
        if ( is_wp_error( $kb_group) ) {
            AMGR_Logging::add_log("Cannot get KB Group by name.", $kb_group);
            return null;
        }

        if ( ! empty($kb_group) && $kb_group_name == $kb_group->name ) {
	        AMGR_Logging::add_log( "'" . $kb_group_name . "' group already exists.");
	        return null;
        }
    ...
    public function rename_group( $kb_id, $kb_group_id, $kb_group_name ) {
	    ...
	    if ( ! current_user_can('admin_eckb_access_manager_page') ) {
		    AMGR_Logging::add_log("Attempt unauthorized access to Access Manager page", $kb_group_id);
		    return false;
	    }
	    ...
	    if ( $is_group_public || $is_group_public === null ) {
		    AMGR_Logging::add_log("Cannot rename PUBLIC group: " . $kb_group_id, $kb_id);
	        return false;
	    }
    ...
    public function delete_group( $kb_id, $kb_group_id ) {
	    ...
	    $is_group_public = epkb_get_instance()->db_kb_public_groups->is_public_group( $kb_id, $kb_group_id );
	    if ( $is_group_public || $is_group_public === null ) {
		    AMGR_Logging::add_log("Cannot delete PUBLIC group: " . $kb_group_id, $kb_id);
		    return false;
	    }
	    ...
	    if ( $all_groups === null ) {
		    AMGR_Logging::add_log("Cannot get current groups. KB ID " . $kb_id, $kb_group_id);
		    return false;
	    }
        ...
        if ( is_wp_error( $kb_group) ) {
            AMGR_Logging::add_log("Cannot get KB Group by primary key: ", $kb_group_id);
            return false;
        }
        if ( empty($kb_group) || $kb_group->kb_group_id != $kb_group_id ) {
	        AMGR_Logging::add_log("Delete Group: group is not in the table ", $kb_group_id);
	        return false;
        }

        // 2. delete all users for this KB Group
        $kb_group_users = new AMGR_DB_KB_Group_Users();
        if ( ! $kb_group_users->remove_users_from_group( $kb_group_id ) ) {
	        AMGR_Logging::add_log("Could not delete users in this KB Group ", $kb_group_id);
	        return false;
        }

	    // 3. delete group KB Categories from category tables
	    $result = epkb_get_instance()->db_access_kb_categories->delete_group_categories( $kb_id, $kb_group_id );
        if ( $result === false ) {
	        AMGR_Logging::add_log("Could not delete categories for group to be deleted ", $kb_group_id);
	        return false;
        }
	    $result = epkb_get_instance()->db_access_read_only_categories->delete_group_categories( $kb_id, $kb_group_id );
	    if ( $result === false ) {
		    AMGR_Logging::add_log("Could not delete read-only categories for group to be deleted ", $kb_group_id);
		    return false;
	    }
	    $result = epkb_get_instance()->db_access_read_only_articles->delete_group_articles( $kb_id, $kb_group_id );
	    if ( $result === false ) {
		    AMGR_Logging::add_log("Could not delete read-only aticles for group to be deleted ", $kb_group_id);
		    return false;
	    }
        ...
        if ( is_wp_error( $deletion) ) {
            AMGR_Logging::add_log("Cannot get KB Group by ID: ", $kb_group_id);
            return false;
        }
	    if ( $deletion !== null ) {
		    AMGR_Logging::add_log("Remove Group: record still in the table", $kb_group_id);
		    return false;
	    }
	 ...
	 * @return bool
	 */
    public function is_group_from_kb( $kb_id, $kb_group_id, $log_error = true ) {
