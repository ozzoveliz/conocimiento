...
class AMGR_KB_Config_Elements {
	...
	public function radio_button( $args = array() ) {
		...
		<div class="config-input-group <?php echo esc_html( $args['input_group_class'] )?>" id="<?php echo $id ?>_group">
			...
			<label class="<?php echo esc_html( $args['label_class'] )?>" for="<?php echo $id ?>"><?php echo esc_html( $args['label'] )?></label>
				...
				<ul>					<?php
					...
					foreach( $args['options'] as $key => $label ) {
						...
						if( $args['current'] == $label ) {
							$checked_list = 'amgr-radio-checked';
						}						?>
	 ...
	 * @return string
	 */
	public function checkboxes_multi_select( $args = array(), $is_multi_select_not=false ) {
	...
	public function kb_setting_box( $args ){
		...
		?>
		<div id="<?php echo $args[ 'box_id' ]; ?>" class="amgr-settings-box">
			<div class="amgr-sb-title">
				<h4><?php echo $args[ 'heading' ]; ?></h4>
				<span class="amgr-sb-info">
				    <a href="<?php echo $args[ 'link_url' ]; ?>" target="_blank"><span class="ep_font_icon_info"></span></a>
			    </span>
			</div>
			<div class="amgr-sb-content">
