...
class EMKB_Utilities {
	...
	public static function get_post_status_text( $post_status ) {
		...
		$post_statuses = array( 'draft' => __( 'Draft' ), 'pending' => __( 'Pending', 'echo-knowledge-base' ),
	...
	public static function get_bottom_notice_message_box($message, $title='', $type='success', $extra_html='' ) {
			...
			foreach ( $EZSQL_ERROR as $error ){
				...
				foreach ( $amgr_tables as $table_name ) {
					if ( !empty($error['error_str']) && strpos($error['error_str'], $table_name) !== false ) {
						...
						EMKB_Logging::add_log( 'Database error', $EZSQL_ERROR );
		...
		$title = empty($title) ? '' : '<h4>' . esc_html( $title ) . '</h4>';
			...
			<form class="epkb-dbf__form"<?php echo empty( $values['form_method'] ) ? '' : ' method="' . esc_attr( $values['form_method'] ) . '"'; ?>>				<?php
				if ( isset($values['form_inputs']) ) {
					foreach ( $values['form_inputs'] as $input ) {
						echo '<div class="epkb-dbf__form__input">' . wp_kses( $input, self::get_allowed_html_tags( ['image', 'link', 'form'] ) ) . '</div>';
					}
				} ?>
			</form>
	...
	public static function dialog_box( $values ) { ?>
		...
		<div id="<?php echo esc_attr( $values[ 'id' ] ); ?>" class="epkb-dialog-box-form">
			...
			<!---- Body ---->
			<div class="epkb-dbf__body">				<?php
				echo empty( $values['body']) ? '' : wp_kses_post( $values['body'] ); ?>
			</div>
	...
	public static function sanitize_get_id( $id ) {
		...
		if ( empty( $id) || is_wp_error($id) ) {
			EMKB_Logging::add_log( 'Error occurred (01)' );
			return new WP_Error('E001', 'invalid ID' );
		}

		if ( is_array( $id) ) {
			if ( ! isset( $id['id']) ) {
				EMKB_Logging::add_log( 'Error occurred (02)' );
				return new WP_Error('E002', 'invalid ID' );
			}

			$id_value = $id['id'];
			if ( ! self::is_positive_int( $id_value ) ) {
				EMKB_Logging::add_log( 'Error occurred (03)', $id_value );
		...
		if ( ! self::is_positive_int( $id ) ) {
			EMKB_Logging::add_log( 'Error occurred (04)', $id );
	...
	public static function retrieve_and_sanitize_form( $form, $all_fields_specs ) {
		...
		foreach( $submitted_fields as $submitted_key => $submitted_value ) {
			...
			if ( $field_type == EMKB_Input_Filter::WP_EDITOR ) {
				$sanitized_fields[$submitted_key] = wp_kses( $submitted_value, self::get_allowed_html_tags( ['all'] ) );

			} elseif ( $field_type == EMKB_Input_Filter::TYPOGRAPHY ) {
				$sanitized_fields[$submitted_key] = EMKB_Input_Filter::sanitize_typography( $submitted_value );

			} elseif ( $field_type == EMKB_Input_Filter::TEXT && ! empty($all_fields_specs[$submitted_key]['allowed_tags']) ) {
	...
	private static function post_sanitize( $key, $default = '', $value_type = 'text', $max_length = 0 ) {
		...
		if ( $value_type == 'text-area' ) {
			...
			$value = wp_kses( $_POST[$key], self::get_allowed_html_tags( ['all'] ) );
		} else {
	...
	private static function get_sanitize( $key, $default = '', $value_type = 'text', $max_length = 0 ) {
		...
		if ( $value_type == 'text-area' ) {
			...
			$value = wp_kses( $_GET[$key], self::get_allowed_html_tags( ['all'] ) );
		} else {
	...
	public static function get_wp_option( $option_name, $default, $is_array=false, $return_error=false ) {
		...
		if ( $return_error && $option === null && ! empty($wpdb->last_error) ) {
			...
			EMKB_Logging::add_log( "DB failure: " . $wpdb_last_error, 'Option Name: ' . $option_name );
	...
	public static function save_wp_option( $option_name, $option_value, $sanitized ) {
		...
		if ( $sanitized !== true ) {
			return new WP_Error( '433', __( 'Error occurred', 'echo-knowledge-base' ) . ' ' . $option_name );
		}
		...
		if ( $result === false ) {
			EMKB_Logging::add_log( 'Failed to update option', $option_name );
	...
	public static function get_postmeta( $post_id, $meta_key, $default, $is_array=false, $return_error=false ) {
		...
		if ( $return_error && $option === null && ! empty($wpdb->last_error) ) {
			$wpdb_last_error = $wpdb->last_error;   // add_log changes last_error so store it first
			EMKB_Logging::add_log( "DB failure: " . $wpdb_last_error, 'Meta Key: ' . $meta_key );
			return new WP_Error(__( 'Error occurred', 'echo-knowledge-base' ), $wpdb_last_error);
		}
	...
	public static function save_postmeta( $post_id, $meta_key, $meta_value, $sanitized ) {
		...
		if ( $result === null && ! empty($wpdb->last_error) ) {
			$wpdb_last_error = $wpdb->last_error;   // add_log changes last_error so store it first
			EMKB_Logging::add_log( "DB failure: " . $wpdb_last_error );
			return new WP_Error(__( 'Error occurred', 'echo-knowledge-base' ), $wpdb_last_error);
		}
		...
		if ( empty($result) ) {
			if ( false === $wpdb->query( $wpdb->prepare( "INSERT INTO $wpdb->postmeta (`meta_key`, `meta_value`, `post_id`) VALUES (%s, %s, %d)", $meta_key, $serialized_value, $post_id ) ) ) {
				EMKB_Logging::add_log("Failed to insert meta data. ", $meta_key);
			...
			if ( false === $wpdb->query( $wpdb->prepare( "UPDATE $wpdb->postmeta SET meta_value = %s WHERE meta_key = '%s' AND post_id = %d", $serialized_value, $meta_key, $post_id ) ) ) {
				EMKB_Logging::add_log("Failed to update meta data. ", $meta_key);
				return new WP_Error( '33', __( 'Error occurred', 'echo-knowledge-base' ) );
			}
		}

		if ( $result === false ) {
			EMKB_Logging::add_log( 'Failed to update meta key', $meta_key );
	...
	public static function delete_postmeta( $post_id, $meta_key ) {
		...
		if ( false === $wpdb->query( $wpdb->prepare( "DELETE FROM $wpdb->postmeta WHERE post_id = %d and meta_key = '%s'", $post_id, $meta_key ) ) ) {
			EMKB_Logging::add_log("Could not delete post '" . self::get_variable_string($meta_key) . "'' metadata: ", $post_id);
			return false;
		}
	...
	public static function get_inline_style( $styles, $config ) {
		...
		foreach( $style_array as $style ) {
			...
			if ( count($key_value) == 2 && isset($key_value[1]) ) {
				...
				$typography_values = array_merge( EMKB_Typography::$typography_defaults, $config[$key_value[2]] );
	...
	public static function get_typography_config( $typography ) {
		...
		if ( empty( $typography ) || ! is_array( $typography ) ) {
			$typography = EMKB_Typography::$typography_defaults;
		}
	...
	public static function contact_us_for_feedback() {
		return ' ' .  esc_html_x( "We'd love to hear your feedback!", 'echo-knowledge-base' ) . ' ' .
		       '<a href="https://www.echoknowledgebase.com/feature-request/" target="_blank" rel="noopener noreferrer">' . esc_html_x( 'click here', 'echo-knowledge-base' ) . '</a>';
	}
	...
	public static function send_email( $message, $to_support_email='', $reply_to_email='', $reply_to_name='', $subject='' ) {
		...
		if ( empty( $message ) || strlen( $message ) > 5000 ) {
			EMKB_Logging::add_log( 'Invalid or too long email message', $message );
		...
		add_filter( 'wp_mail_content_type', array( 'EMKB_Utilities', 'set_html_content_type' ), 999 );

		// send email
		$result = wp_mail( $to_support_email, $subject, $message, $headers );

		// remove filter that allows HTML in the email content
		remove_filter( 'wp_mail_content_type', array( 'EMKB_Utilities', 'set_html_content_type' ), 999 );

		return $result == false ? __( 'Failed to send email.', 'echo-knowledge-base' ) : '';
	}
	 ...
	 *
	 * For example: EMKB_Utilities::ajax_show_error_die( EMKB_Utilities::report_generic_error( 411 ) );
	 *
	 ...
	 */
	public static function ajax_verify_nonce_and_admin_permission_or_error_die( $wpnonce_name='_wpnonce_emkb_ajax_action', $context='' ) {
		...
		if ( ! EMKB_KB_Core::is_user_access_to_context_allowed( $context ) ) {
			self::ajax_show_error_die(__( 'You do not have permission to edit this knowledge base', 'echo-knowledge-base' ) . ' (E02)');
		}
	}
	...
	public static function get_allowed_html_tags( $contexts=[] ) {
		...
		add_filter( 'safe_style_css', array( 'EMKB_Utilities', 'safe_inline_css_properties' ) );
		...
		// Audio
		if ( current_user_can( 'unfiltered_html' ) && ( in_array( 'audio', $contexts ) || in_array( 'all', $contexts ) ) ) {
			$tags_list['audio']     = self::get_allowed_html_attributes( 'audio' );
			$tags_list['source']    = self::get_allowed_html_attributes( 'source' );
		}

		// Video
		if ( current_user_can( 'unfiltered_html' ) && ( in_array( 'video', $contexts ) || in_array( 'all', $contexts ) ) ) {
			$tags_list['video']     = self::get_allowed_html_attributes( 'video' );
			$tags_list['source']    = self::get_allowed_html_attributes( 'source' );
		}

		// Form
		if ( current_user_can( 'unfiltered_html' ) && ( in_array( 'form', $contexts ) || in_array( 'all', $contexts ) ) ) {
		...
		// Style
		if ( current_user_can( 'unfiltered_html' ) && ( in_array( 'style', $contexts ) || in_array( 'all', $contexts ) ) ) {
			$tags_list['style']     = self::get_allowed_html_attributes( 'script' );
		}

		// Script
		if ( current_user_can( 'unfiltered_html' ) && ( in_array( 'script', $contexts ) || in_array( 'all', $contexts ) ) ) {
			$tags_list['link']      = self::get_allowed_html_attributes( 'script' );
			$tags_list['script']    = self::get_allowed_html_attributes( 'script' );
		}

		// Iframe
		if ( current_user_can( 'unfiltered_html' ) && ( in_array( 'iframe', $contexts ) || in_array( 'all', $contexts ) ) ) {
	 ...
	 * @return string
	 */
	public static function get_active_theme_classes( $prefix = '' ) {
