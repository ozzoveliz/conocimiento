 ...
 */
class EMKB_Add_Ons_Page {

	public function __construct() {
		add_filter( EMKB_KB_Core::EMKB_KB_LICENSE_FIELD, array( $this, 'display_license_status_update') );
		add_action( 'wp_ajax_emkb_handle_license_request', array( $this, 'handle_license_request' ) );
		add_action( 'wp_ajax_nopriv_emkb_handle_license_request', array( 'EMKB_Utilities', 'user_not_logged_in' ) );
	}
	...
	public function display_license_status_update( $output ) {
		...
		$output .= '<div class="ekb-add-on-title">'
					.'<h2>'. EMKB_License_Handler::PLUGIN_NAME . '</h2>' .
			       '</div>'.
		           '<div class="ekb-license-check" id="emkb_license_check">' . __( 'Updating license status. Please wait.', 'echo-knowledge-base' ) . '</div>' .
		           '<div id="emkb-ajax-in-progress" style="display:none;">' . esc_html__( 'Updating license...', 'echo-knowledge-base' ) . '</div>';
		$output .= '</li>';
	...
	public function handle_license_request() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			EMKB_Utilities::ajax_show_error_die( __( 'You do not have permission to view license info.', 'echo-knowledge-base' ) );
		}

		$emkb_instance = emkb_get_instance();
		if ( empty($emkb_instance) || ! $emkb_instance instanceof Echo_Multiple_Knowledge_Bases ) {
			EMKB_Utilities::ajax_show_error_die( __( 'Failed to initialize (M99).', 'echo-knowledge-base' ) );
		}

		$command = EMKB_Utilities::post('command');
		$command = ! empty($command) && in_array( $command, array( 'get_license_info', 'save' ) ) ? $command : '';
		if ( empty($command) ) {
			EMKB_Utilities::ajax_show_error_die( __( 'Unknown licensing command: ', 'echo-knowledge-base' ) . $command );
		}

		if ( empty( $emkb_instance->license_handler ) ) {
			EMKB_Utilities::ajax_show_error_die(  __( 'Failed to initialize (L99)', 'echo-knowledge-base' ) );
		}

		$license_handler = $emkb_instance->license_handler;
	 ...
	 *
	 * @param EMKB_License_Handler $license_handler
	 * @param $license_data
	...
	private function get_license_state( $license_handler, $license_data ) {
		...
		if ( ! empty($add_on_data) &&  is_object( $add_on_data ) && ! empty( $add_on_data->new_version ) ) {
			...
			EMKB_Logging::add_log("Could not contact licensing server.", ( isset($add_on_data->license) ? $add_on_data->license : '' ) );
		...
		if ( ! empty($license_error) && $license_handler->is_connection_error( $license_error ) ) {
			EMKB_Logging::add_log("Could not contact licensing server.", ( isset($license_data->license) ? $license_data->license : '' ) );
			$cached_info = $license_handler->get_cached_info();
		...
		EMKB_Utilities::save_wp_option( EMKB_License_Handler::EMKB_LICENSE_STATE, array( $status_msg, $license_state, $latest_add_on_version, $expiry_date ), true );

		EMKB_Utilities::save_wp_option('emkb_last_license_check', array(), true);
	...
	private function license_fields( $status, $status_msg, $expiry_date, $latest_add_on_version, $license_key ) {
		...
		if ( emkb_get_instance()->is_dormant ) {
		...
		<div class="ekb-license-key">
			<ul>
				...
				<li><input type="text" name="<?php echo EMKB_License_Handler::LICENSE_KEY_OPTION_ID; ?>" id="<?php echo EMKB_License_Handler::LICENSE_KEY_OPTION_ID; ?>"
				...
				<li><span class="emkb_license_action"><input id="emkb_save_btn" class="primary-btn" type="button" name="emkb_save_btn" value="<?php echo  __( 'Save', 'echo-knowledge-base' ); ?>"/>
				<input type="hidden" id="_wpnonce_emkb_license_key" name="_wpnonce_emkb_license_key" value="<?php echo wp_create_nonce( "_wpnonce_emkb_license_key" ); ?>"/></span></li>
			</ul>
		</div>
		...
		<div class="ekb-license-status">
			<ul>
				...
				<li><span id="emkb_license_key_status"><?php echo wp_kses_post( $status_msg ); ?></span></li>
			</ul>
		</div>
		...
		<div class="ekb-license-expiry">			<?php
			...
			if ( ! empty($expiry_date) ) {
				$expiry_msg = emkb_get_instance()->license_handler->get_expiry_warning( $expiry_date );
				$expiry_icon = empty($expiry_msg) ? 'ep_font_icon_checkmark' : 'ep_font_icon_error_circle';
			}
			$expiry_msg = empty($expiry_msg) ? EMKB_Utilities::get_formatted_datetime_string( $expiry_date ) : $expiry_msg;			?>
		...
		<div class="ekb-license-version">			<?php
			...
			$new_version_available = version_compare( Echo_Multiple_Knowledge_Bases::$version, $latest_add_on_version, "<" );
			...
			<ul>
				...
				<li><?php _e( 'Installed:', 'echo-knowledge-base' ); ?> <?php echo Echo_Multiple_Knowledge_Bases::$version; ?></li>
				...
				<li>					<?php
					...
					if ( $new_version_available && $status == 'valid' ) {
						$name = plugin_basename( Echo_Multiple_Knowledge_Bases::$plugin_file );
