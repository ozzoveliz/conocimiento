...
class ELAY_License_Handler {
	...
	const PLUGIN_NAME           = 'Elegant Layouts';
	const LICENSE_KEY_OPTION_ID = 'elay_license_key';
	const ELAY_LICENSE_STATE    = 'elay_license_state';
	...
	function __construct( $_file, $_version ) {
		...
		$this->license_key = ELAY_Utilities::get_wp_option( self::LICENSE_KEY_OPTION_ID, '' );
		$this->setup_hooks();
	}
	...
	private function setup_hooks() {
		...
		add_filter( ELAY_KB_Core::ELAY_KB_ADD_ON_LICENSE_MSG, array( $this, 'get_license_status_short') );
	}
	...
	public function auto_updater() {
		...
		// set up the updater
		if ( ! class_exists( 'ELAY_EDD_SL_Plugin_Updater' ) ) {
			require_once 'class-elay-edd-sl-plugin-updater.php';
		}

		new ELAY_EDD_SL_Plugin_Updater(
	...
	public function get_cached_info() {
		$default_array = array( __( 'Unknown status.', 'echo-knowledge-base' ), '', Echo_Elegant_Layouts::$version, '' );
		$elay_license_state = ELAY_Utilities::get_wp_option( self::ELAY_LICENSE_STATE, $default_array, true );
		$status_msg = isset($elay_license_state[0]) ? $elay_license_state[0] : __( 'Unknown status.', 'echo-knowledge-base' );
		$license_state = isset($elay_license_state[1]) ? $elay_license_state[1] : '';
		$latest_add_on_version = isset($elay_license_state[2]) ? $elay_license_state[2] : Echo_Elegant_Layouts::$version;
		$expiry_date = isset($elay_license_state[3]) ? $elay_license_state[3] : '';
	...
	public function handle_license_command() {
		...
		if ( ! isset( $_REQUEST['_wpnonce_elay_license_key'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce_elay_license_key'], '_wpnonce_elay_license_key' ) ) {
			ELAY_Utilities::ajax_show_error_die( __( 'Security check failed.', 'echo-knowledge-base' ) );
		}

		// need license key to activate/deactivate the license
		$entered_license_key = empty($_REQUEST['elay_license_key']) ? '' : sanitize_text_field( $_REQUEST['elay_license_key'] );
	...
	private function save_license( $license_key ) {
		...
		$result = ELAY_Utilities::save_wp_option( self::LICENSE_KEY_OPTION_ID, $license_key );
		if ( is_wp_error( $result ) ) {
			/* @var $result WP_Error */
			$message = $result->get_error_data();
			if ( empty($message) ) {
				ELAY_Utilities::ajax_show_error_die( $result->get_error_message(),  __( 'Could not save settings.', 'echo-knowledge-base' ) );
			}
			ELAY_Utilities::ajax_show_error_die( $this->generate_error_summary( $result->get_error_data() ), __( 'Settings NOT saved due to following problems:', 'echo-knowledge-base' ) );
		}
	}
	...
	public function contact_license_server( $action, $license_key ) {
		...
		if ( is_wp_error( $response ) ) {
			$error_msg = '<br/>REQUEST: url => ' . $get_url . ', args => ' . ELAY_Utilities::get_variable_string( $get_args );
			$error_msg .= '<br/>RESPONSE: ' . ELAY_Utilities::get_variable_string( $response );
	...
	public function get_expiry_warning( $expiry_date ) {
		...
		$difference_in_days = ELAY_Utilities::get_days_since( '', $expiry_date );
	...
	public function get_license_state( $force_refresh=false ) {
		...
		$stored_license_data = ELAY_Utilities::get_wp_option('elay_last_license_check', array(), true, true);
		if ( is_wp_error($stored_license_data) ) {
			ELAY_Logging::add_log("Could not retrieve license data option.");
			return null;
		}
		...
		$licenses_tab_url = admin_url( 'edit.php?post_type=' . ELAY_KB_Handler::get_post_type( ELAY_KB_Core::DEFAULT_KB_ID ) . '&page=ep' . 'kb-add-ons&ep' . 'kb-tab=licenses' );
		...
		if ( ! empty($license_error) && $this->is_connection_error( $license_error ) ) {
			...
			if ( ! empty($stored_license_data['last_connection_error']) && ELAY_Utilities::get_days_since( $stored_license_data['last_connection_error'] ) > 30 ) {
		...
		$stored_license_data['new_version'] = version_compare( Echo_Elegant_Layouts::$version, $latest_add_on_version, "<" ) == 0 ? null : $latest_add_on_version;
		...
		ELAY_Utilities::save_wp_option( 'elay_last_license_check', $stored_license_data );
