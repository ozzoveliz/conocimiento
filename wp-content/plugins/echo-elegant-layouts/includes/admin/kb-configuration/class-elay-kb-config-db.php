 ...
 */
class ELAY_KB_Config_DB {

	// Prefix for WP option name that stores KB configuration
	const KB_CONFIG_PREFIX =  'elay_config_';
	...
	public function get_kb_configs( $skip_check=false ) {
		...
		if ( ! empty( $this->cached_settings ) && $this->is_cached_all_kbs ) {
			...
			foreach( $this->cached_settings as $config ) {
				...
				$kb_id = $config['id'];
				$kb_options_checked[$kb_id] = wp_parse_args( $config, ELAY_KB_Config_Specs::get_default_kb_config() );
			}
		...
		if ( empty($kb_options) || ! is_array($kb_options) ) {
			ELAY_Logging::add_log("Did not retrieve any kb config. Using defaults (22). Last error: " . $wpdb->last_error, $kb_options);
			$kb_options = array();
		}
		...
		foreach ( $kb_options as $ix => $row ) {
			...
			if ( $config === false ) {
				ELAY_Logging::add_log("Could not unserialize configuration: ", ELAY_Utilities::get_variable_string($row['option_value']));
				continue;
			}

			if ( empty($config) || ! is_array($config) ) {
				ELAY_Logging::add_log("Did not find configuration");
				continue;
			}

			if ( empty($config['id']) ) {
				ELAY_Logging::add_log("Found invalid configuration", $config);
				continue;
			}

			$kb_id = ( $config['id'] === self::DEFAULT_KB_ID ) ? $config['id'] : ELAY_Utilities::sanitize_get_id( $config['id'] );
			if ( is_wp_error($kb_id) ) {
				continue;
			}

			// with WPML we need to trigger hook to have configuration names translated
			if ( ELAY_Core_Utilities::is_wpml_enabled_addon( $kb_id ) ) {
				$config = get_option( self::KB_CONFIG_PREFIX . $kb_id );
				if ( empty($config) || ! is_array($config) ) {
					ELAY_Logging::add_log( "Did not find KB configuration, the default was used instead.", $kb_id );
					$config = array();
				}
			}
	...
	public function get_kb_ids( $ignore_error=false ) {
		...
		$kb_option_names = $wpdb->get_results("SELECT option_name FROM $wpdb->options WHERE option_name LIKE '" . ELAY_KB_Core::ELAY_KB_CONFIG_PREFIX . "%'", ARRAY_A );
		if ( empty($kb_option_names) || ! is_array($kb_option_names) ) {
			if ( ! $ignore_error ) {
				ELAY_Logging::add_log("Did not retrieve any kb config. Try to deactivate and active KB plugin to see if the issue will be fixed (11). Last error: " . $wpdb->last_error, $kb_option_names);
			}
		...
		foreach ( $kb_option_names as $kb_option_name ) {
			...
			$kb_id = str_replace( ELAY_KB_Core::ELAY_KB_CONFIG_PREFIX, '', $kb_option_name['option_name'] );
			$kb_id = ELAY_Utilities::sanitize_int( $kb_id, self::DEFAULT_KB_ID );
			$kb_ids[$kb_id] = $kb_id;
		}
	...
	public function get_kb_config( $kb_id, $return_error=false ) {
		...
		if ( ! empty($this->cached_settings[$kb_id]) ) {
			$config = wp_parse_args( $this->cached_settings[$kb_id], ELAY_KB_Config_Specs::get_default_kb_config() );
			$config['id'] = $kb_id;
			return $config;
		}
		...
		if ( empty($config) || ! is_array($config) ) {
			return $return_error ? new WP_Error('DB231', "Did not find KB configuration. Try to deactivate and reactivate KB plugin to see if this fixes the issue. " . ELAY_Utilities::contact_us_for_support() )
								 : ELAY_KB_Config_Specs::get_default_kb_config();
		}

		// use defaults for missing or empty fields
		$config = wp_parse_args( $config, ELAY_KB_Config_Specs::get_default_kb_config() );
		$config['id'] = $kb_id;
	...
	public function get_kb_config_or_default( $kb_id ) {
		...
		if ( empty( $kb_config ) || ! is_array( $kb_config) || is_wp_error( $kb_config ) ) {
			return ELAY_KB_Config_Specs::get_default_kb_config();
		}
	...
	public function get_value( $kb_id, $setting_name, $default = '' ) {
		...
		$kb_config = empty( $kb_id ) ? ELAY_KB_Config_Specs::get_default_kb_config() : $this->get_kb_config( $kb_id );

		if ( isset( $kb_config[$setting_name] ) ) {
			return $kb_config[$setting_name];
		}

		$default_settings = ELAY_KB_Config_Specs::get_default_kb_config();
	...
	public function update_kb_configuration( $kb_id, array $config, $upsert=true ) {
		...
		$sanitized_config = wp_parse_args( $sanitized_config, ELAY_KB_Config_Specs::get_default_kb_config() );
	...
	private function save_kb_config( array $config, $kb_id, $upsert=true ) {
		...
		// KB configuration always starts with elay_config_[ID]
		...
		if ( $result === false ) {
			ELAY_Logging::add_log( 'Failed to update kb config for kb_id', $kb_id, 'Last DB ERROR: (' . $wpdb->last_error . ')' );
	...
	public function add_new_add_on_config( $kb_id ) {
		$this->update_kb_configuration( $kb_id, ELAY_KB_Config_Specs::get_default_kb_config() );
	}

	/**
	 * Filter KB config to add ELAY values 
	 * @param $kb_config
	 * @return array
	 */
