 ...
 */
class ELAY_KB_Handler {

	// Prefix for custom post type name associated with given KB; this will never change
	const KB_POST_TYPE_PREFIX = ELAY_KB_Core::ELAY_KB_POST_TYPE_PREFIX;  // changing this requires db update
	...
	public static function is_kb_request() {
		...
		$kb_post_type = empty( $_REQUEST['post_type'] ) ? '' : preg_replace( '/[^A-Za-z0-9 \-_]/', '', ELAY_Utilities::request_key( 'post_type' ) );
		$is_kb_post_type = empty($kb_post_type) ? false : self::is_kb_post_type( $kb_post_type );
		if ( $is_kb_post_type ) {
			return true;
		}

		$kb_taxonomy = empty( $_REQUEST['taxonomy'] ) ? '' : preg_replace( '/[^A-Za-z0-9 \-_]/', '', ELAY_Utilities::request_key( 'taxonomy' ) );
	...
	public static function get_post_type( $kb_id ) {
		$kb_id = ELAY_Utilities::sanitize_int($kb_id, ELAY_KB_Core::DEFAULT_KB_ID );
