 ...
 */
class KBLK_KB_Config_DB {

	// Prefix for WP option name that stores KB configuration
	const KB_CONFIG_PREFIX =  'kblk_config_';
	...
	public function get_kb_configs( $skip_check=false ) {
		...
		if ( ! empty( $this->cached_settings ) && $this->is_cached_all_kbs ) {
			...
			foreach( $this->cached_settings as $config ) {
				...
				$kb_id = $config['id'];
				$kb_options_checked[$kb_id] = wp_parse_args( $config, KBLK_KB_Config_Specs::get_default_kb_config() );
			}
		...
		if ( empty( $kb_options ) || ! is_array( $kb_options ) ) {
			KBLK_Logging::add_log( "Did not retrieve any kb config. Using defaults (22). Last error: " . $wpdb->last_error, $kb_options );
			$kb_options = array();
		}
		...
		foreach ( $kb_options as $ix => $row ) {
			...
			if ( $config === false ) {
				KBLK_Logging::add_log("Could not unserialize configuration: ", KBLK_Utilities::get_variable_string($row['option_value']));
				continue;
			}

			if ( empty($config) || ! is_array($config) ) {
				KBLK_Logging::add_log("Did not find configuration");
				continue;
			}

			if ( empty($config['id']) ) {
				KBLK_Logging::add_log("Found invalid configuration", $config);
				continue;
			}

			$kb_id = ( $config['id'] === self::DEFAULT_KB_ID ) ? $config['id'] : KBLK_Utilities::sanitize_get_id( $config['id'] );
			if ( is_wp_error( $kb_id)  ) {
				continue;
			}

			// with WPML we need to trigger hook to have configuration names translated
			if ( KBLK_Core_Utilities::is_wpml_enabled_addon( $kb_id ) ) {
				$config = get_option( self::KB_CONFIG_PREFIX . $kb_id );
				if ( empty( $config ) || ! is_array( $config ) ) {
					KBLK_Logging::add_log( "Did not find KB configuration, the default was used instead.", $kb_id );
					$config = array();
				}
			}
		...
		if ( empty( $kb_options_checked ) || ! isset( $kb_options_checked[self::DEFAULT_KB_ID] ) ) {
			KBLK_Logging::add_log("Need at least default configuration.");
			$kb_options_checked[self::DEFAULT_KB_ID] = KBLK_KB_Config_Specs::get_default_kb_config();
		}
	...
	public function get_kb_ids( $ignore_error=false ) {
		...
		// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		$kb_option_names = $wpdb->get_results("SELECT option_name FROM $wpdb->options WHERE option_name LIKE '" . KBLK_KB_Core::KBLK_KB_CONFIG_PREFIX . "%'", ARRAY_A );
		if ( empty( $kb_option_names ) || ! is_array( $kb_option_names ) ) {
			if ( ! $ignore_error ) {
				KBLK_Logging::add_log("Did not retrieve any kb config. Try to deactivate and active KB plugin to see if the issue will be fixed (11). Last error: " . $wpdb->last_error, $kb_option_names);
			}
		...
		foreach ( $kb_option_names as $kb_option_name ) {
			...
			$kb_id = str_replace( KBLK_KB_Core::KBLK_KB_CONFIG_PREFIX, '', $kb_option_name['option_name'] );
			$kb_id = KBLK_Utilities::sanitize_int( $kb_id, self::DEFAULT_KB_ID );
			$kb_ids[$kb_id] = $kb_id;
		}
	...
	public function get_kb_config( $kb_id, $return_error=false ) {
		...
		$kb_id = ( $kb_id === self::DEFAULT_KB_ID ) ? $kb_id : KBLK_Utilities::sanitize_get_id( $kb_id );
		if ( is_wp_error( $kb_id ) ) {
			return $return_error ? $kb_id : KBLK_KB_Config_Specs::get_default_kb_config();
		}
		/** @var int $kb_id */

		// retrieve settings if already cached
		if ( ! empty( $this->cached_settings[$kb_id] ) ) {
			$config = wp_parse_args( $this->cached_settings[$kb_id], KBLK_KB_Config_Specs::get_default_kb_config() );
			$config['id'] = $kb_id;
			return $config;
		}
		...
		if ( empty( $config ) || ! is_array( $config ) ) {
			// if new KB then this will happen: KBLK_Logging::add_log("Did not find KB configuration (DB231).", $kb_id);
			return $return_error ? new WP_Error('DB231', "Did not find KB configuration. Try to deactivate and reactivate KB plugin to see if this fixes the issue. " . KBLK_Utilities::contact_us_for_support() )
								 : KBLK_KB_Config_Specs::get_default_kb_config();
		}

		// use defaults for missing or empty fields
		$config = wp_parse_args( $config, KBLK_KB_Config_Specs::get_default_kb_config() );
		$config['id'] = $kb_id;
	...
	public function get_kb_config_or_default( $kb_id ) {
		...
		if ( empty( $kb_config ) || ! is_array( $kb_config ) || is_wp_error( $kb_config ) ) {
			return KBLK_KB_Config_Specs::get_default_kb_config();
		}
	...
	public function get_value( $kb_id, $setting_name, $default = '' ) {
		...
		$kb_config = empty($kb_id) ? KBLK_KB_Config_Specs::get_default_kb_config() : $this->get_kb_config( $kb_id );

		if ( isset( $kb_config[$setting_name] ) ) {
			return $kb_config[$setting_name];
		}

		$default_settings = KBLK_KB_Config_Specs::get_default_kb_config();
	...
	public function update_kb_configuration( $kb_id, array $config, $upsert=true ) {
		...
		$fields_specification = KBLK_KB_Config_Specs::get_fields_specification();
		$input_filter = new KBLK_Input_Filter();
		$sanitized_config = $input_filter->validate_and_sanitize_specs( $config, $fields_specification );
		if ( is_wp_error( $sanitized_config ) ) {
			KBLK_Logging::add_log( 'Could not update the configuration', $kb_id, $sanitized_config );
			return $sanitized_config;
		}

		// use defaults for missing configuration
		$sanitized_config = wp_parse_args( $sanitized_config, KBLK_KB_Config_Specs::get_default_kb_config() );
	...
	private function save_kb_config( array $config, $kb_id, $upsert=true ) {
		...
		// KB configuration always starts with kblk_config_[ID]
		...
		$upsert_sql = $upsert ? "ON DUPLICATE KEY UPDATE `option_name` = VALUES(`option_name`), `option_value` = VALUES(`option_value`), `autoload` = VALUES(`autoload`)" : '';
		$result = $wpdb->query( $wpdb->prepare( "INSERT INTO $wpdb->options (`option_name`, `option_value`, `autoload`) VALUES (%s, %s, %s) " . esc_sql( $upsert_sql ), $option_name, $serialized_config, 'no' ) );
		if ( $result === false ) {
			KBLK_Logging::add_log( 'Failed to update kb config for kb_id', $kb_id, 'Last DB ERROR: (' . $wpdb->last_error . ')' );
	...
	public function add_new_add_on_config( $kb_id ) {
		$this->update_kb_configuration( $kb_id, KBLK_KB_Config_Specs::get_default_kb_config() );
	}

	/**
	 * Filter KB config to add KBLK values 
	 * @param $kb_config
	 * @return array
	 */
