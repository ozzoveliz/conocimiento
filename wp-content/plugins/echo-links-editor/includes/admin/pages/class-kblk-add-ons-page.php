 ...
 */
class KBLK_Add_Ons_Page {

	public function __construct() {
		add_filter( KBLK_KB_Core::KBLK_KB_LICENSE_FIELD, array( $this, 'display_license_status_update') );
		add_action( 'wp_ajax_kblk_handle_license_request', array( $this, 'handle_license_request' ) );
		add_action( 'wp_ajax_nopriv_kblk_handle_license_request', array( 'KBLK_Utilities', 'user_not_logged_in' ) );
	}
	...
	public function display_license_status_update( $output ) {
		...
		$output .= '<div class="ekb-add-on-title">'
					.'<h2>'. KBLK_License_Handler::PLUGIN_NAME . '</h2>' .
			       '</div>'.
		           '<div class="ekb-license-check" id="kblk_license_check">Updating license status. Please wait.</div>' .
		           '<div id="kblk-ajax-in-progress" style="display:none;">' . esc_html__( 'Updating license...', 'echo-knowledge-base' ) . '</div>';
		$output .= '</li>';
	...
	public function handle_license_request() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			KBLK_Utilities::ajax_show_error_die( 'You do not have permission to view license info.' );
		}

		$kblk_instance = kblk_get_instance();
		if ( empty($kblk_instance) || ! $kblk_instance instanceof Echo_Links_Editor ) {
			KBLK_Utilities::ajax_show_error_die( 'Failed to initialize (M99).' );
		}

		$command = KBLK_Utilities::post('command');
		$command = ! empty($command) && in_array( $command, array( 'get_license_info', 'save' ) ) ? $command : '';
		if ( empty($command) ) {
			KBLK_Utilities::ajax_show_error_die( 'Unknown licensing command: ' . $command );
		}

		if ( empty( $kblk_instance->license_handler ) ) {
			KBLK_Utilities::ajax_show_error_die( 'Failed to initialize (L99)' );
		}

		$license_handler = $kblk_instance->license_handler;
	 ...
	 *
	 * @param KBLK_License_Handler $license_handler
	 * @param $license_data
	...
	private function get_license_state( $license_handler, $license_data ) {
		...
		if ( ! empty($license_error) && $license_handler->is_connection_error( $license_error ) ) {
			KBLK_Logging::add_log("Could not contact licensing server.", ( isset($license_data->license) ? $license_data->license : '' ) );
			$cached_info = $license_handler->get_cached_info();
		...
		KBLK_Utilities::save_wp_option('kblk_last_license_check', array(), true);
	...
	private function license_fields( $status, $status_msg, $expiry_date, $latest_add_on_version, $license_key ) {
		...
		<div class="ekb-license-key">
			<ul>
				...
				<li><input type="text" name="<?php echo KBLK_License_Handler::LICENSE_KEY_OPTION_ID; ?>" id="<?php echo KBLK_License_Handler::LICENSE_KEY_OPTION_ID; ?>"
				...
				<li><span class="kblk_license_action"><input id="kblk_save_btn" class="primary-btn" type="button" name="kblk_save_btn" value="<?php echo  __( 'Save', 'echo-knowledge-base' ); ?>"/>
						<input type="hidden" id="_wpnonce_kblk_license_key" name="_wpnonce_kblk_license_key" value="<?php echo wp_create_nonce( "_wpnonce_kblk_license_key" ); ?>"/></span></li>
			</ul>
		</div>
		...
		<div class="ekb-license-status">
			<ul>
				...
				<li><span id="kblk_license_key_status"><?php echo wp_kses_post( $status_msg ); ?></span></li>
			</ul>
		</div>
		...
		<div class="ekb-license-expiry">			<?php
			...
			$expiry_msg = empty($expiry_msg) ? KBLK_Utilities::get_formatted_datetime_string( $expiry_date ) : $expiry_msg;			?>
		...
		<div class="ekb-license-version">			<?php
			...
			$new_version_available = version_compare( Echo_Links_Editor::$version, $latest_add_on_version, "<" );
			...
			<ul>
				...
				<li>Installed: <?php echo Echo_Links_Editor::$version; ?></li>
				...
				<li>					<?php
					...
					if ( $new_version_available && $status == 'valid' ) {
						$name = plugin_basename( Echo_Links_Editor::$plugin_file );
