...
class KBLK_Search_Query {
	...
	private $kb_id = KBLK_KB_Config_DB::DEFAULT_KB_ID;
	 ...
	 * @return array|false
	 */
	public function display_article_link_in_search( $result /** unused */, $kb_id, $filtered_user_input ) {
		return $this->kb_search_articles( $kb_id, $filtered_user_input, [], 20 );
	}
	...
	public function kb_search_articles( $kb_id, $filtered_user_input, $user_category_ids, $batch_size, $results_page_num=0 ) {
		...
		$this->audit( 'is_link_editor_enabled', KBLK_Utilities::is_link_editor_enabled() );

		$this->is_wpml_enabled = KBLK_Core_Utilities::is_wpml_enabled_addon( $this->kb_id );
		$this->audit( 'is_wpml_enabled', $this->is_wpml_enabled );

		$this->is_amag = KBLK_Utilities::is_amag_on();
		$this->audit( 'is_amag', $this->is_amag );

		// get individual search keywords;
		$this->search_keywords = KBLK_Search_Query_Extras::get_search_keywords( $this->kb_id, $filtered_user_input );
		...
		if ( ! empty($this->user_category_ids) && $this->is_amag) {
			...
			foreach( $this->user_category_ids as $user_category_id ) {
				$user_selected_category = KBLK_Core_Utilities::get_kb_category_unfiltered( $this->kb_id, $user_category_id );
			...
			$user_filtered_categories = KBLK_KB_Core::filter_user_categories( $user_selected_categories );
		...
		// search Tags if any left; not KBLK
		$tag_result = array();
		...
		// allow users to modify search results
		if ( has_filter( 'kblk_search_results' ) ) {
			$search_result_new = apply_filters( 'kblk_search_results', $search_result );
			if ( ! empty( $search_result_new ) && is_array( $search_result_new ) ) {
				$search_result = $search_result_new;
			}
		}
	...
	private function search_articles( $return_total, $results_from=0, $initial_limit=0 ) {
		...
		if ( $this->is_wpml_enabled ) {
			...
			$articles_seq_data = KBLK_Utilities::get_kb_option( $this->kb_id, KBLK_KB_Core::KBLK_ARTICLES_SEQUENCE, array(), true );
		...
		if ( $this->is_amag ) {
			$search_result = KBLK_KB_Core::found_posts( $search_result );
		}
	...
	private function generate_article_search_query( $return_total, $results_from, $initial_limit ) {
		...
		$kb_post_type  = KBLK_KB_Handler::get_post_type( $this->kb_id );
		...
		/** FROM clause */
		$from = " FROM $post_tbl wp " . ( KBLK_Utilities::is_link_editor_enabled() ? " LEFT JOIN $post_meta_tbl wpp ON ( wp.ID = wpp.post_id ) " : '' );
		...
		/** GROUP BY clause */
		$group_by = $return_total || ! KBLK_Utilities::is_link_editor_enabled() ? '' : " GROUP BY wp.ID  ";
	...
	private function where_like_keywords() {
		...
		foreach ( $this->search_keywords as $search_keyword ) {
			...
			if ( in_array($search_keyword, KBLK_Search_Query_Extras::html_css_keywords()) ) {
			...
			if ( KBLK_Utilities::is_link_editor_enabled() ) {
	...
	private function assemble_order_by() {
		...
		// try AND with OR for up to 5 keywords
	...
	private function search_tags( $results_from=0, $limit=0 ) {
		...
		$tags_search_keywords_no_filler = KBLK_Search_Query_Extras::get_search_keywords( $this->kb_id, $this->sanitized_raw_user_input, false );
		...
		$tags_search_keywords = KBLK_Search_Query_Extras::get_search_keywords( $this->kb_id, $this->sanitized_raw_user_input );
		$this->audit( 'tags_search_keywords (1)', $tags_search_keywords );

		// get individual search keywords
		$kb_tag_taxonomy = KBLK_KB_Handler::get_tag_taxonomy_name( $this->kb_id );
		$kb_category_taxonomy = KBLK_KB_Handler::get_category_taxonomy_name( $this->kb_id );
		...
		if ( $this->is_wpml_enabled ) {
			...
			$current_article_ids = KBLK_Utilities::is_wpml_article_active( $search_result, true );
