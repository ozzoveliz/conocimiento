 ...
 */
class KBLK_Search_Query_Extras {
	...
	public static function get_search_keywords( $kb_id, $sanitized_raw_user_input, $filter_stop_words=true ) {
		...
		foreach ( $keywords[0] as $keyword ) {
			...
			$keyword = KBLK_Utilities::mb_strtolower( $keyword ); // does not work with å etc.: strtolower($keyword);
		...
		if ( empty($filtered_search_keywords) || count($filtered_search_keywords) > KBLK_Search_Query::MAX_KEY_WORDS ) {
			$filtered_search_keywords = array( $sanitized_raw_user_input );
		}
	...
	public static function html_css_keywords() {
		$words = array( "align", "background", "block", "border", "bottom", "button", "center", "class", "code", "color", "column", "content", "data", "display", "end", "family", "font",
		...
		return apply_filters( 'kblk_html_css_search_filter', $words );
	}
}
