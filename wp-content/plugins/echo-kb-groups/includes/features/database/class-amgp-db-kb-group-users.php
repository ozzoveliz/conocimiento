 ...
 */
class AMGP_DB_KB_Group_Users extends AMGP_DB  {
     ...
     * @var string
     */
    public $cache_user = AMGP_KB_Core::DB_KB_GROUP_USERS;
    ...
    public function __construct() {
        ...
        $this->table_name  = $wpdb->prefix . AMGP_KB_Core::DB_KB_GROUP_USERS;
	...
	public function add_group_user( $kb_id, $wp_user_id, $kb_group_id, $kb_role_name ) {
		...
		if ( ! current_user_can('admin_eckb_access_manager_page') ) {
			AMGP_Logging::add_log("Attempt unauthorized access to Access Manager page", $wp_user_id);
			return false;
		}
		...
		if ( ! AMGP_Utilities::is_positive_int($wp_user_id) ) {
			AMGP_Logging::add_log("WP User ID is not valid", $wp_user_id);
			return false;
		}
		...
		if ( ! AMGP_KB_Role::is_valid_role( $kb_role_name ) ) {
			AMGP_Logging::add_log("KB Role Name is not valid", $kb_role_name);
			return false;
		}

		// check that the KB Group exists
		$result = amgp_get_instance()->db_kb_groups->is_group_from_kb( $kb_id, $kb_group_id );
		if ( ! $result ) {
			return false;
		}

		// only Administrators can add KB Managers
		if ( ! current_user_can('manage_options') && $kb_role_name == AMGP_KB_Role::KB_ROLE_MANAGER ) {
			AMGP_Logging::add_log("KB Role Name is not valid. Administrator is required to add KB Managers.", $kb_role_name);
			return false;
		}
		...
		if ( ! $wp_user ) {
			AMGP_Logging::add_log("Found user that is not existing WP user.", $wp_user_id);
			return false;
		}
		...
		if ( is_wp_error( $kb_group_user) ) {
			AMGP_Logging::add_log("Cannot add user to the KB Group. User ID: ", $wp_user_id . ', KB Group ID: ' . $kb_group_id . ', KB Role: ' . $kb_role_name);
			return false;
		}
		if ( ! empty($kb_group_user) ) {
			// not a permanent error: AMGP_Logging::add_log("Found existing record already.", $wp_user_id . ', ' . $kb_group_id . ', ' . $kb_role_name);
			return false;
		}

		// 2. ensure the user is not KB Manager
		$kb_managers_id = AMGP_Groups::get_kb_managers();
		if ( $kb_managers_id === null ) {
			AMGP_Logging::add_log("Error retrieving KB Managers.", $kb_id);
			return false;
		}
		if ( in_array($wp_user_id, $kb_managers_id ) ) {
			AMGP_Logging::add_log("The user is already KB Manager.", $wp_user_id);
			return false;
		}
	...
	public function get_user_role_config( $kb_id, $kb_group_id, $wp_user_id ) {
		...
		if ( is_wp_error( $kb_group_user) ) {
			AMGP_Logging::add_log("Cannot retrieve user role for KB Group. KB ID: ", $kb_id . ', KB Group ID: ' . $kb_group_id . ', wp_user_id: ' . $wp_user_id);
			return null;
		}
		...
		if ( ! AMGP_KB_Role::is_valid_role( $kb_group_user->kb_role_name ) ) {
			return null;
		}
	...
	public function get_user_role( $kb_id, $kb_group_id ) {
		...
		if ( ! AMGP_WP_ROLES::use_kb_groups() ) {
			AMGP_Logging::add_log( 'Internal error (15)' );
			return null;
		}
		...
		if ( is_wp_error( $kb_group_user) ) {
			AMGP_Logging::add_log("Cannot retrieve user role for KB Group. KB ID: ", $kb_id . ', KB Group ID: ' . $kb_group_id . ', wp_user_id: ' . $user->ID);
			return null;
		}
		if ( $kb_group_user !== null ) {
			return AMGP_KB_Role::is_valid_role( $kb_group_user->kb_role_name ) ? $kb_group_user->kb_role_name : null;
		}

		// 2. otherwise determine KB Role based on mapping of WP Role to KB, KB Group and WP User ID
		$wp_kb_user_role = AMGP_WP_Roles::get_user_highest_kb_role_based_on_wp_role( $kb_id, $user, true );
    ...
    public function get_group_users_config( $kb_id, $kb_group_id ) {
	    ...
	    if ( is_wp_error($group_users) ) {
		    AMGP_Logging::add_log( "Cannot retrieve users for a group " . $kb_group_id . " for KB " . $kb_id);
		    return null;
	    }
    ...
    public function get_group_role_users_config( $kb_id, $kb_group_id, $kb_role_name ) {
	    ...
	    if ( is_wp_error($group_users) ) {
		    AMGP_Logging::add_log( "Cannot retrieve users for a group " . $kb_group_id . " for KB " . $kb_id . " and role " . $kb_role_name);
		    return null;
	    }
	...
	public function get_groups_for_given_user( $kb_id ) {
		...
		// KB Group membership takes precedence
		$user_groups = $this->get_rows_by_where_clause( array('kb_id' => $kb_id, 'wp_user_id' => $user->ID) );
		if ( is_wp_error($user_groups) ) {
			AMGP_Logging::add_log( "Cannot retrieve records for user " . $user->ID . " for KB " . $kb_id);
			return null;
		}
		$user_groups = $user_groups === null ? array() : $user_groups;
		$user_groups_ids = AMGP_Access_Utilities::get_group_ids( $user_groups );
	...
	public function get_user_kbs() {
		...
		if ( is_wp_error($user_groups) ) {
			AMGP_Logging::add_log( "Cannot retrieve kb records for user " . $user->ID );
			return null;
		}
		$user_groups = $user_groups === null ? array() : $user_groups;
		...
		$wp_map_user_kbs = AMGP_WP_Roles::get_user_kbs( $user );
	...
	public function remove_group_user( $kb_group_id, $wp_user_id ) {
		...
		if ( ! current_user_can('admin_eckb_access_manager_page') ) {
			AMGP_Logging::add_log("Attempt unauthorized access to Access Manager page for user " . $wp_user_id, $kb_group_id);
			return false;
		}

		// validate input
		if ( ! AMGP_Utilities::is_positive_int($kb_group_id) ) {
			AMGP_Logging::add_log("WP User ID is not valid", $kb_group_id);
			return null;
		}
		...
		if ( is_wp_error( $kb_group_user) ) {
			AMGP_Logging::add_log("Cannot get KB Group user: ", $kb_group_user);
			return false;
		}
		if ( $kb_group_user === null ) {
			//AMGP_Logging::add_log("Remove Group User: record is not in the table", $wp_user_id);
			return false;
		}

		/* $wp_user = get_userdata( $kb_group_user->wp_user_id );
		if ( ! $wp_user ) {
			AMGP_Logging::add_log("Found user that is not existing WP user.", $kb_group_user->wp_user_id);
			return false;
		} */
		...
		if ( $kb_group_user !== null ) {
			AMGP_Logging::add_log("Remove Group User: record still in the table", $kb_group_user);
			return false;
		}
	...
	public function remove_user_from_all_groups( $wp_user_id ) {
		...
		if ( ! AMGP_Utilities::is_positive_int($wp_user_id) ) {
			AMGP_Logging::add_log("KB Group ID is not valid", $wp_user_id);
			return false;
		}
		...
		if ( $deletion !== null && count($deletion) > 0 ) {
			AMGP_Logging::add_log("Remove User from all Groups: record still in the table", $wp_user_id);
			return false;
		}
    ...
    public function remove_users_from_group( $kb_group_id ) {
	    ...
	    if ( $deletion !== null && count($deletion) > 0 ) {
		    AMGP_Logging::add_log("Remove Users from Group: record still in the table", $kb_group_id);
		    return false;
	    }
