 ...
 */
class AMGP_DB_Access_KB_Categories extends AMGP_DB  {
     ...
     * @var string
     */
    public $cache_group = AMGP_KB_Core::DB_ACCESS_KB_CATEGORIES;
    ...
    public function __construct() {
        ...
        $this->table_name  = $wpdb->prefix . AMGP_KB_Core::DB_ACCESS_KB_CATEGORIES;
    ...
    public function get_category_group_ids( $kb_id, $kb_category_id ) {
	    ...
	    $kb_groups_ids = AMGP_Access_Utilities::get_group_ids( $kb_groups );
	    ...
	    foreach( $kb_category_groups as $kb_category_group ) {
		    ...
		    if ( ! in_array($kb_category_group->kb_group_id, $kb_groups_ids) ) {
			    AMGP_Logging::add_log( "Found KB Group does not belong to the KB " . $kb_category_id . " for KB " . $kb_id, $kb_category_group );
			    return null;
		    }
	 ...
	 */
	 public function get_kb_categories( $kb_id ) {
		...
		if ( is_wp_error($kb_categories) ) {
			AMGP_Logging::add_log("Failed to get categories.", 'KB ID: ' . $kb_id);
			return null;
		}
	...
	public function get_kb_category( $kb_category_id ) {
		...
		if ( ! AMGP_Utilities::is_positive_int( $kb_category_id ) ) {
			AMGP_Logging::add_log("KB Category ID is not valid", $kb_category_id);
			return null;
		}

		$kb_category = $this->get_rows_by_where_clause( array('kb_category_id' => $kb_category_id) );
		if ( is_wp_error($kb_category) ) {
			AMGP_Logging::add_log("Failed to invoke select for KB Category: ", $kb_category_id);
			return null;
		}
		...
		if ( count($kb_category) > 1 ) {
			AMGP_Logging::add_log( "Found more than one  KB Group for given category " . $kb_category_id, $kb_category );
			return null;
		}
