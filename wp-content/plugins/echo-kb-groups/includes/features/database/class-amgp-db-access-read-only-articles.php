 ...
 */
class AMGP_DB_Access_Read_Only_Articles extends AMGP_DB  {
     ...
     * @var string
     */
    public $cache_group = AMGP_KB_Core::DB_ACCESS_READ_ARTICLES;
    ...
    public function __construct() {
        ...
        $this->table_name  = $wpdb->prefix . AMGP_KB_Core::DB_ACCESS_READ_ARTICLES;
    ...
    public function get_read_only_article_groups( $kb_id, $kb_article_id ) {
        ...
        if ( ! AMGP_Utilities::is_positive_int( $kb_article_id ) ) {
            AMGP_Logging::add_log("Post ID is not valid", $kb_article_id);
            return null;
        }

	    $kb_articles = $this->get_rows_by_where_clause( array('kb_id' => $kb_id, 'kb_article_id' => $kb_article_id) );
	    if ( is_wp_error($kb_articles) ) {
		    AMGP_Logging::add_log( "Cannot retrieve article " . $kb_article_id . " for KB " . $kb_id );
		    return null;
	    }
	...
	public function get_all_read_only_kb_articles_ids( $kb_id ) {
		...
		if ( is_wp_error($kb_articles_ids) ) {
			AMGP_Logging::add_log( "Cannot retrieve articles for given KB", $kb_id, $kb_articles_ids );
			return null;
		}
	...
	public function get_group_read_only_articles_ids( $kb_id, $kb_group_id ) {
		...
		if ( is_wp_error($group_articles) ) {
			AMGP_Logging::add_log( "Cannot retrieve articles for group " . $kb_group_id . " for KB " . $kb_id );
			return null;
		}
	...
	public function is_article_read_only_for_group( $kb_id, $kb_group_id, $kb_article_id ) {
		...
		if ( ! AMGP_Utilities::is_positive_int($kb_article_id) ) {
			AMGP_Logging::add_log("Article ID is not valid", $kb_article_id);
			return false;
		}
	...
	public function add_read_only_group_to_article( $kb_id, $kb_group_id, $kb_article_id ) {
		...
		$has_access = AMGP_Access_Utilities::is_user_role_in_group_same_or_higher( $kb_id, $kb_group_id, AMGP_KB_Role::KB_ROLE_CONTRIBUTOR );
		...
		if ( is_wp_error( $result) ) {
			AMGP_Logging::add_log("Cannot add KB Group to KB Article. KB ID: ", $kb_id . ', KB Group ID: ' . $kb_group_id . ', article ID: ' . $kb_article_id);
			return false;
		}
		if ( $result === null ) {
			AMGP_Logging::add_log("KB Group ID was not found", $kb_group_id);
			return false;
		}
	...
	public function delete_article( $kb_id, $kb_article_id ) {
		...
		if ( $deleted !== null && count($deleted) > 0 ) {
			AMGP_Logging::add_log("Delete article from read-only articles table: article still in the table");
			return false;
		}
	...
	public function delete_group_articles( $kb_id, $kb_group_id ) {
		...
		if ( $group_article_ids === null ) {
			AMGP_Logging::add_log("Add Group: failed to retrieve group articles", $kb_group_id);
			return false;
		}
		...
		if ( is_wp_error($deleted) ) {
			AMGP_Logging::add_log( "DB error: cannot delete all articles for KB Group " . $kb_group_id );
			return false;
		} else if ( ! empty($deleted) ) {
			AMGP_Logging::add_log("Cannot delete all articles for KB Group " . $kb_group_id);
			return false;
		}
		...
		if ( $is_group_public ) {
			...
			$result = AMGP_Access_Utilities::update_status_of_deleted_category_articles( $kb_id, $group_article_ids );
			if ( empty($result) ) {
				return false;
			}
		}
