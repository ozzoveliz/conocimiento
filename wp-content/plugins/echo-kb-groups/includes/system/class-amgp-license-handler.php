...
class AMGP_License_Handler {
	...
	public function get_cached_info() {
		$default_array = array( esc_html__( 'Unknown status.', 'echo-knowledge-base' ), '', Echo_KB_Groups::$version, '' );
		$amgp_license_state = AMGP_Utilities::get_wp_option( self::AMGP_LICENSE_STATE, $default_array, true );
		$status_msg = isset($amgp_license_state[0]) ? $amgp_license_state[0] : esc_html__( 'Unknown status.', 'echo-knowledge-base' );
	...
	public function handle_license_command() {
		...
		if ( ! isset( $_REQUEST['_wpnonce_amgp_license_key'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['_wpnonce_amgp_license_key'] ) ), '_wpnonce_amgp_license_key' ) ) {
			AMGP_Utilities::ajax_show_error_die( esc_html__( 'Security check failed.', 'echo-knowledge-base' ) );
		}
	...
	private function save_license( $license_key ) {
		...
		if ( is_wp_error( $result ) ) {
			...
			if ( empty($message) ) {
				AMGP_Utilities::ajax_show_error_die( $result->get_error_message(),  esc_html__( 'Could not save settings.', 'echo-knowledge-base' ) );
			}
			AMGP_Utilities::ajax_show_error_die( $this->generate_error_summary( $result->get_error_data() ), esc_html__( 'Settings NOT saved due to following problems:', 'echo-knowledge-base' ) );
		}
	}
