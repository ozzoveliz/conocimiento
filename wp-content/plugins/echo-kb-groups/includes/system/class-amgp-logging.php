...
class AMGP_Logging {
	...
	const LOGGING_OPTION_NAME =  'amgp_error_log';
	...
	private static function add_log_now( $error_message, $param1='', $param2='' ) {
		...
		$variable = AMGP_Utilities::get_variable_string( $variable );
		...
		// prepare error message
		$error_message = AMGP_Utilities::substr( $error_message, 0, 3000);
		...
		$kb_id = defined('EM'.'KB_PLUGIN_NAME') ? $kb_id : AMGP_KB_Config_DB::DEFAULT_KB_ID;

		// add new error log entry but remove oldest one if more than max
		// FUTURE TODO log current user
		$error_log[] = array( 'plugin' => AMGP_PLUGIN_NAME, 'kb' => $kb_id, 'date' => date("Y-m-d H:i:s"), 'message' => $error_message, 'trace' => $stack_trace );
		...
		// save the error log
		AMGP_Utilities::save_wp_option( self::LOGGING_OPTION_NAME, $error_log, true );
	}
	...
	private static function can_log_message() {
		...
		$is_debug_on = true; // TODO AMGP_Utilities::get_wp_option( 'amgp_debug', false );
		if ( $is_debug_on !== true ) {
			return false;
		}
		...
		$orig_report_on_error = self::$report_on_error;
		self::$report_on_error = false;
		$kb_version = AMGP_Utilities::get_wp_option( 'amgp_version', null, false, true );
		self::$report_on_error = $orig_report_on_error;
	...
	public static function get_logs() {
		$logs = AMGP_Utilities::get_wp_option( self::LOGGING_OPTION_NAME, array(), true );
	...
	public static function generateStackTrace()	{
		...
		foreach( debug_backtrace() as $trace ) {
			...
			$file = AMGP_Utilities::substr($file, 1, 500);
		...
		$stackMsg = AMGP_Utilities::substr( $stackMsg, 0, 2000);
