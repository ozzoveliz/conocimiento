 ...
 */
class AMGP_Add_Ons_Page {

	public function __construct() {
		add_filter( AMGP_KB_Core::AMGP_KB_LICENSE_FIELD, array( $this, 'display_license_status_update') );
		add_action( 'wp_ajax_amgp_handle_license_request', array( $this, 'handle_license_request' ) );
		add_action( 'wp_ajax_nopriv_amgp_handle_license_request', array( $this, 'user_not_logged_in' ) );
	}
	...
	public function display_license_status_update( $output ) {
		...
		$info = 'Enter your license key for the ' . AMGP_License_Handler::PLUGIN_NAME . ' add-on.';
		$output .= '<li class="ekb-add-on">';
		$output .= '<div class="ekb-add-on-title">'
					.'<h2>'. AMGP_License_Handler::PLUGIN_NAME . '</h2>' .
			            '<div class="amgp_license_info">' .
		                   '<span class="ep_font_icon_info"></span><p class="ep_font_icon_info_content hidden">' . esc_html($info) . '</p>' .
		                '</div>'.
			       '</div>'.
		           '<div class="ekb-license-check" id="amgp_license_check">Updating license status. Please wait.</div>' .
		           '<div id="amgp-ajax-in-progress" style="display:none;">' . esc_html__( 'Updating license...', 'echo-knowledge-base' ) . '</div>';
		$output .= '</li>';
	...
	public function handle_license_request() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			AMGP_Utilities::ajax_show_error_die( 'You do not have permission to view license info.' );
		}

		$amgp_instance = amgp_get_instance();
		if ( empty($amgp_instance) || ! $amgp_instance instanceof Echo_KB_Groups ) {
			AMGP_Utilities::ajax_show_error_die( 'Failed to initialize (M99).' );
		}

		$command = AMGP_Utilities::post('command');
		$command = ! empty($command) && in_array( $command, array( 'get_license_info', 'save' ) ) ? $command : '';
		if ( empty($command) ) {
			AMGP_Utilities::ajax_show_error_die( 'Unknown licensing command: ' . $command );
		}

		if ( empty( $amgp_instance->license_handler ) ) {
			AMGP_Utilities::ajax_show_error_die( 'Failed to initialize (L99)' );
		}

		$license_handler = $amgp_instance->license_handler;
	 ...
	 *
	 * @param AMGP_License_Handler $license_handler
	 * @param $license_data
	...
	private function get_license_state( $license_handler, $license_data ) {
		...
		if ( ! empty($license_error) && $license_handler->is_connection_error( $license_error ) ) {
			//AMGP_Logging::add_log("Could not contact licensing server.", ( isset($license_data->license) ? $license_data->license : '' ) );
			$cached_info = $license_handler->get_cached_info();
		...
		AMGP_Utilities::save_wp_option('amgp_last_license_check', array(), true);
	...
	private function license_fields( $status, $status_msg, $expiry_date, $latest_add_on_version, $license_key ) {
		...
		<div class="ekb-license-version">			<?php
			...
			$new_version_available = version_compare( Echo_KB_Groups::$version, $latest_add_on_version, "<" );
			...
			<ul>
				...
				<li>					<?php
					...
					if ( $new_version_available && $status == 'valid' ) {
						$name = plugin_basename( Echo_KB_Groups::$plugin_file );
