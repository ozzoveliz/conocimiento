 ...
 */
class AMCR_KB_Config_DB {

	// Prefix for WP option name that stores KB configuration
	const KB_CONFIG_PREFIX =  'amcr_config_';
	...
	function get_kb_configs( $skip_check=false ) {
		...
		if ( ! empty($this->cached_settings) && $this->is_cached_all_kbs ) {
			...
			foreach( $this->cached_settings as $config ) {
				...
				$kb_id = $config['id'];
				$kb_options_checked[$kb_id] = wp_parse_args( $config, AMCR_KB_Config_Specs::get_default_kb_config() );
			}
		...
		if ( empty($kb_options) || ! is_array($kb_options) ) {
			AMCR_Logging::add_log("Did not retrieve any kb config. Using defaults", $kb_options);
			$kb_options = array();
		}
		...
		foreach ( $kb_options as $ix => $row ) {
			...
			if ( $config === false ) {
				AMCR_Logging::add_log("Could not unserialize configuration");
				continue;
			}

			if ( empty($config) || ! is_array($config) ) {
				AMCR_Logging::add_log("Did not find configuration");
				continue;
			}

			if ( count($config) < 1 ) {
				AMCR_Logging::add_log("Found KB configuration is incomplete", count($config));
			}

			if ( empty($config['id']) ) {
				AMCR_Logging::add_log("Found invalid configuration", $config);
				continue;
			}

			$kb_id = ( $config['id'] === self::DEFAULT_KB_ID ) ? $config['id'] : AMCR_Utilities::sanitize_get_id( $config['id'] );
		...
		if ( empty($kb_options_checked) || ! isset($kb_options_checked[self::DEFAULT_KB_ID]) ) {
			AMCR_Logging::add_log("Need at least default configuration.");
			$kb_options_checked[self::DEFAULT_KB_ID] = AMCR_KB_Config_Specs::get_default_kb_config();
		}
	...
	public function get_kb_ids() {
		...
		$kb_option_names = $wpdb->get_results("SELECT option_name FROM $wpdb->options WHERE option_name LIKE '" . AMCR_KB_Core::AMCR_KB_CONFIG_PREFIX . "%'", ARRAY_A );
		if ( empty($kb_option_names) || ! is_array($kb_option_names) ) {
			AMCR_Logging::add_log("Did not retrieve any kb config. Using defaults", $kb_option_names);
		...
		foreach ( $kb_option_names as $kb_option_name ) {
			...
			$kb_id = str_replace( AMCR_KB_Core::AMCR_KB_CONFIG_PREFIX, '', $kb_option_name['option_name'] );
			$kb_id = AMCR_Utilities::sanitize_int( $kb_id, self::DEFAULT_KB_ID );
			$kb_ids[$kb_id] = $kb_id;
		}
	...
	public function get_kb_config( $kb_id, $default_on_error=true ) {
		...
		$kb_id = ( $kb_id === self::DEFAULT_KB_ID ) ? $kb_id : AMCR_Utilities::sanitize_get_id( $kb_id );
		if ( is_wp_error($kb_id) ) {
			return $kb_id;
		}
		/** @var int $kb_id */
		...
		if ( ! empty($this->cached_settings[$kb_id]) ) {
			$config = wp_parse_args( $this->cached_settings[$kb_id], AMCR_KB_Config_Specs::get_default_kb_config() );
			$config['id'] = $kb_id;
			return $config;
		}
		...
		if ( empty($config) || ! is_array($config) ) {
			AMCR_Logging::add_log("Did not find AMCR configuration (DB23).", $kb_id);
			return ( $default_on_error ? AMCR_KB_Config_Specs::get_default_kb_config() : new WP_Error('get_kb_config', 'Did not find KB Config') );
		}

		if ( count($config) < 1 ) {
			AMCR_Logging::add_log("Found AMCR configuration is incomplete", count($config));
		}

		// use defaults for missing or empty fields
		$config = wp_parse_args( $config, AMCR_KB_Config_Specs::get_default_kb_config() );
		$config['id'] = $kb_id;
	...
	public function get_kb_config_or_default( $kb_id ) {
		...
		if ( is_wp_error( $kb_config ) ) {
			return AMCR_KB_Config_Specs::get_default_kb_config();
		}
	...
	public function get_value( $kb_id, $setting_name, $default = '' ) {
		...
		$kb_config = empty($kb_id) ? AMCR_KB_Config_Specs::get_default_kb_config() : $this->get_kb_config( $kb_id );
		if ( is_wp_error( $kb_config ) ) {
			AMCR_Logging::add_log( "Could not retrieve KB configuration (15). Settings name: ", $setting_name, $kb_config );
			return $default;
		}
		...
		$default_settings = AMCR_KB_Config_Specs::get_default_kb_config();
	...
	public function update_kb_configuration( $kb_id, array $config, $upsert=true ) {
		...
		$fields_specification = AMCR_KB_Config_Specs::get_fields_specification();
		...
		if ( is_wp_error($sanitized_config) ) {
			AMCR_Logging::add_log( 'Could not update AMCR configuration', $kb_id, $sanitized_config );
			return $sanitized_config;
		}

		$sanitized_config = wp_parse_args( $sanitized_config, AMCR_KB_Config_Specs::get_default_kb_config() );
	...
	private function save_kb_config( array $config, $kb_id, $upsert=true ) {
		...
		// KB configuration always starts with amcr_config_[ID]
		...
		if ( $result === false ) {
			AMCR_Logging::add_log( 'Failed to update kb config for kb_id', $kb_id );
	...
	public function add_new_add_on_config( $kb_id ) {
		...
		$this->update_kb_configuration( $kb_id, AMCR_KB_Config_Specs::get_default_kb_config() );
