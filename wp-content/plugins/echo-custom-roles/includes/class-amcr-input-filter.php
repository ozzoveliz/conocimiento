...
class AMCR_Input_Filter {
	...
	public function validate_and_sanitize_specs( array $input, array $specification ) {
		...
		foreach ($input as $key => $input_value) {
			...
			if ( is_wp_error($result) ) {
                ...
                AMCR_Logging::add_log( 'Please change the value of ' . $field_spec['label'] . ' field. ' . $result->get_error_message(), $result );
	...
	private function filter_number( $number, $field_spec=array() ) {
		...
		$number_int = AMCR_Utilities::sanitize_int( $number, null );
		if ( $number != $number_int ) {
			return new WP_Error('filter_not_number', 'The value "' . AMCR_Utilities::get_variable_string( $number ) . '" is not a number.');
		}
	...
	private function filter_id( $id ) {
		$id = AMCR_Utilities::sanitize_get_id( $id );
		if ( is_wp_error($id) ) {
