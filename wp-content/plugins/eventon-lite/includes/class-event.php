...
class EVO_Event extends EVO_Data_Store{
	...
	// time and date related
		...
		// process start and end times according to processes and timezones +2.2.12
		...
		// if the event is live right now - @u2.2.12
	...
	// repeating events
		...
		// next repeat instance that is current (not past) u2.2.13
		...
		function get_repeat_header_html(){
			...
			echo "<div class='evose_repeat_header'><p><span class='title'>".
				esc_attr( evo_lang('This is a repeating event') ) . wp_kses_post( $title_adds ) . "</span>";
			...
			if($ri>0){ 
				...
				if($prev_unixs && isset($prev_unixs[0]) && $prev_unixs[0] > 0){
					...
					echo "<a href='". esc_url( $prev_link )."' class='prev' title='" . esc_attr( $text ). "'><b class='fa fa-angle-left'></b><em>". esc_attr( $text )."</em></a>";
				}				
			...
			if($ri<$repeat_count){
				...
				if($next_unixs && isset($next_unixs[0])){
					...
					echo "<a href='". esc_attr( $next_link )."' class='next' title='". esc_attr( $text )."' data-su='". esc_attr( $next_unix )."'><em>". esc_attr( $text )."</em><b class='fa fa-angle-right'></b></a>";
		...
		function get_tax_ids(){
			...
			if($R && count($R)>0){
				foreach($R as $B){
					...
					foreach($R1 as $C){
						$O1 = $wpdb->get_results( 
							$wpdb->prepare("SELECT op.option_value FROM {$wpdb->prefix}options AS op WHERE op.option_name ='evo_et_taxonomy_%d'", $C->term_id)
						);
