	...
	function get_schema($EventData, $_eventcard){
		...
		if(!empty($schema_jsonld) && $schema_jsonld){
			...
			// offers field
				$_schema_offers = '';
				if( $EVENT->get_prop('_seo_offer_price') && $EVENT->get_prop('_seo_offer_currency')){
					$_schema_offers = ',"offers":{"@type":"Offer","price":"'. $EVENT->get_prop('_seo_offer_price') .'","priceCurrency":"'. esc_html( $EVENT->get_prop('_seo_offer_currency') ).'","availability":"http://schema.org/InStock","validFrom":"'. esc_html( $_schema_starttime ) .'","url":"'. esc_url( $EVENT->get_permalink() ).'"}';
				}
