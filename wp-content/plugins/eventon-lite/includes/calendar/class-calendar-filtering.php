...
class EVO_Cal_Filering{
	...
	// HTML Calendar header filter and sort content
		public function get_content($args, $sortbar=true){
			...
			echo "<div class='eventon_sorting_section' style='display:". esc_attr( $SO_display ) ."'>";
			...
			echo "<div class='eventon_filter_line ".($selectfilterType?'selecttype':'')."'>";
				...
				foreach($_filter_array as $ff=>$vv){ // vv = vv etc.
					...
					// past and future filtering
						if($ff == 'evpf'){
							...
							echo "<div class='eventon_filter evo_hideshow_pastfuture' data-filter_field='". esc_attr( $vv )."' 
								data-filter_val='". esc_attr( $__filter_val ) ."' data-filter_type='custom' data-fldef='". esc_attr( $__filter_val ) ."'>";
								echo "<div class='eventon_filter_selection'>";
									echo "<p class='filtering_set_val' data-opts='evs4_in'>". esc_html( evo_lang('Past and Future Events') ). "<em class='fa fa-check'></em></p>";
									echo "<div class='eventon_filter_dropdown evo_hideshow select_one' style='display:none'>";

									echo "<p class='evo_filter_val ". ($__filter_val =='all'? 'select ':'')."all' data-filter_val='all'>". esc_html( $__text_all_ ) ."</p>";
									echo "<p class='evo_filter_val past ". ( $__filter_val=='all' || $__filter_val=='past'? 'select':'')."' data-filter_val='past'>". esc_html( evo_lang('Only Past Events') ) ."</p>";
									echo "<p class='evo_filter_val future ". ( $__filter_val=='all'|| $__filter_val=='future'? 'select':'')."' data-filter_val='future'>". esc_html( evo_lang('Only Future Events') ) ."</p>";
								echo "</div>
					...
					// virtual event filtering
						if($ff == 'evvir'){
							...
							echo "<div class='eventon_filter evo_hideshow_vir' data-filter_field='". esc_attr( $vv )."' 
								data-filter_val='". esc_attr( $__filter_val ). "' data-filter_type='custom' data-fldef='". esc_attr( $__filter_val )."'>";							
								echo "<div class='eventon_filter_selection'>";
									echo "<p class='filtering_set_val' data-opts='evs4_in'>". esc_html( evo_lang('Virtual Events') ) ."<em class='fa fa-check'></em></p>";
									echo "<div class='eventon_filter_dropdown evo_hideshow select_one' style='display:none'>";
									...
									foreach(array(
										...
										echo "<p class='evo_filter_val ". esc_attr( $f )." ". ( $__filter_val=='all' || $__filter_val== $f ? 'select':'')."' data-filter_val='". esc_attr( $f )."'>". esc_html( evo_lang( $v ) ) ."</p>";
									}	
					...
					// event status filtering
						if($ff == 'evst'){
							...
							echo "<div class='eventon_filter evo_hideshow_st' data-filter_field='". esc_attr( $vv )."' 
								data-filter_val='". esc_attr( $__filter_val) ."' data-filter_type='custom' data-fldef='". esc_attr( $__filter_val) ."'>";						
								echo "<div class='eventon_filter_selection'>";
									echo "<p class='filtering_set_val' data-opts='evs4_in'>". esc_html( evo_lang('Events Status') )."<em class='fa fa-check'></em></p>";
									echo "<div class='eventon_filter_dropdown evo_hideshow select_one' style='display:none'>";
					...
					// Event Tags filtering
						if($ff == 'evotag' ){
							...
							*/
							...
							echo "<div class='eventon_filter evo_hideshow_evotag' data-filter_field='event_tag' data-filter_val='". esc_attr( $__filter_val) ."' data-filter_type='tax' data-fldef='". esc_attr( $__filter_val) ."'>								
								
								<div class='eventon_filter_selection'>
									<p class='filtering_set_val' data-opts='evs4_in'>". esc_html( evo_lang('Event Tag') ). "<em class='fa fa-check'></em></p>
					...
					// filtering value filter is set to show
						if($cats ){
							...
							if(!$filter_show_set_only && $_FO != 'NOT'){
								$inside .=  "<p class='evo_filter_val ". ($_V == 'all'? 'select':'')." all' data-filter_val='all'>". esc_html( $__text_all_ ) ."</p>";
							}
							...
							// each taxonomy term
								foreach($cats as $ct){
									...
									if(!$selectfilterType){
										...
										$inside .=  "<p class='evo_filter_val ". esc_attr( $select ." ".$vv.'_'.$ct->term_id.' '.$ct->slug.' '. ($icon_str?'has_icon':''). ($par?'':' np') ) . "' data-filter_val='".$ct->term_id."' data-filter_slug='".$ct->slug."'>". $icon_str . $term_name."</p>";
							...
							echo "<div class='eventon_filter evo_sortOpt evo_sortList_". esc_attr( $vv) ."' data-filter_field='". esc_attr( $vv) ."' data-filter_val='". esc_attr( $raw_filter_val) ."' data-filter_type='tax' data-fl_o='". esc_attr( $_FO) ." ' data-fldef='". esc_attr( $raw_filter_val ) ."'>
									<p class='filtering_set_val'>". esc_html( $lang__ ) ."<em class='fa fa-check'></em></p>
									<div class='eventon_filter_dropdown' style='display:none'>". wp_kses_post( $inside ) ."</div>	
							</div>";					
				...
				if($selectfilterType){
					echo "<p class='evo_filter_submit'>". esc_html( $this->cal->lang('evcal_lang_apply_filters','Apply Filters') )."</p>";
				}

				// clear filters
				if( !empty($args['filter_clear']) && $args['filter_clear']=='yes' ){
					echo "<p class='evo_filter_clear'>". esc_html( evo_lang('Clear Filters') ) ."</p>";
				}

				// (---) Hook for addon
				do_action('eventon_sorting_filters', $content);
