...
<div class='evo_metarow_health evorow evcal_evdata_row'>
	<span class='evcal_evdata_icons'><i class='fa <?php echo esc_html( get_eventON_icon('evcal__fai_health', 'fa-heartbeat',$evOPT ) );?>'></i></span>
	<div class='evcal_evdata_cell'>
		<h3 class='evo_h3'><?php echo esc_html( evo_lang('Health Guidelines for this Event') );?></h3>
		<div class='evcal_cell'>
			...
			foreach(apply_filters('evo_healthcaredata_frontend', array(
				...
				echo "<div class='evo_health_b_o'><div class='evo_health_b'>";
			 	if($v[0]=='svg')
					echo "<svg class='evo_svg_icon' enable-background='new 0 0 20 20' height='20' viewBox='0 0 512 512' width='20' xmlns='http://www.w3.org/2000/svg'>". wp_kses_post( EVO()->elements->svg->get_icon_path( $v[1]) ) ."</svg>";
				if($v[0]=='i') echo "<i class='fa fa-". esc_attr( $v[1] ). "'></i>";
				echo "<span>". wp_kses_post( $v[2] ) ."</span>
				</div></div>";
			}
			...
			<div class='evo_health_b ehb_other'>
				<span class='evo_health_bo_title'>
					<i class='fa fa-laptop-medical'></i><span><?php esc_html( evo_lang_e('Other Health Guidelines') );?></span>
				</span>
				<span class='evo_health_bo_data'><?php echo wp_kses_post( $EVENT->get_eprop('_health_other') );?></span>
			</div>
			<?php endif;?>
