...
class Evo_Calendar_Now{
	...
	public function get_body( $delay = false ){
		...
		<div class='evo_eventon_now' style='display:<?php echo $hide_now? 'none':'block';?>'>
			<h3><?php echo esc_html( evo_lang('Events Happening Now') );?></h3>
			...
			// now				
				...
				if( count($event_list_array) >0 ){
					...
					echo wp_kses_post( EVO()->calendar->body->get_calendar_header($header_args) ) ;		

					foreach( $event_data as $ED){
						$now_event_ids[] = $ED['event_id'];
						echo wp_kses_post( $ED['content'] );
					}

					echo wp_kses_post( EVO()->calendar->body->get_calendar_footer( true) );

				// no events
				}else{
					echo "<div class='evo_eventon_no_events_now'>". wp_kses_post( EVO()->calendar->helper->get_no_event_content() ) . "</div>";
	...
	public function print_shell_footer( ){
		...
		?><div class='evo_data' data-d='<?php echo esc_attr( wp_json_encode($this->A) );?>'></div></div><?php
	}		
}