...
class evo_sinevent{
	...
	public function __construct(){
		...
		if( isset($_GET['l'])) $this->L = sanitize_text_field($_GET['l']);
	...
	// @~2.6.13
		function oneevent_wrapper(){
			...
			?>
			<div id='evcal_single_event_<?php echo esc_attr( get_the_ID() );?>' class='ajde_evcal_calendar eventon_single_event evo_sin_page<?php echo ' '. esc_attr( $etsty );?>' data-eid='<?php echo esc_attr( $event_id);?>' data-l='<?php echo esc_attr( $this->L );?>' data-j='<?php echo wp_json_encode($json);?>'>
			<?php
		}
		function oneevent_evodata(){
			// deprecating ?><div class='evo-data' <?php echo esc_attr( $this->get_evo_data() );?>></div>
			<div class='evo_cal_data' data-sc='<?php echo wp_json_encode($this->get_cal_event_JSON());?>'></div>
			<?php
		}
		function oneevent_head(){
			...
			?><div id='evcal_head' class='calendar_header'><p id='evcal_cur'><?php echo esc_html($header_text);?></p></div><?php
		}
		...
		function oneevent_event_data(){
			...
			echo   esc_attr( $content[0]['content'] );
		}
	...
	// page content
		function page_content(){
			...
			if( (!$onlylogged_cansee || ($onlylogged_cansee && is_user_logged_in() ) ) && 
				...
				echo esc_html( "<p>".evo_lang('You must login to see this event')."<br/><a class='button' href=". esc_url( wp_login_url() ) ." title='".  evo_lang('Login')."'>". evo_lang('Login') ."</a></p>" );
			}
	...
	// get repeat event page header
		function repeat_event_header($ri, $eventid){	
			...
			echo "<div class='evose_repeat_header'><p><span class='title'>". esc_html( evo_lang('This is a repeating event') ). "</span>";
			...
			if($ri>0){ 
				...
				if($prev_unixs && isset($prev_unixs[0]) && $prev_unixs[0] > 0){
					...
					echo esc_html( "<a href='". esc_url( $prev_link ) ."' class='prev' title='". esc_attr( $text ) ."'><b class='fa fa-angle-left'></b><em>". esc_html( $text )."</em></a>");
				}				
			...
			if($ri<$repeat_count){
				...
				if($next_unixs && isset($next_unixs[0])){
					...
					echo esc_html( "<a href='{$next_link}' class='next' title='{$text}'><em>{$text}</em><b class='fa fa-angle-right'></b></a>" );
