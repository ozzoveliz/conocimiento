...
class EVO_Shortcodes {
	...
	// single events
		function single_event_box($atts){
				...
				if( is_array($fields)){
					...
					foreach($fields as $ff){
						...
						case 'event_stime':
							...
							echo esc_attr( $EVENT->get_readable_formatted_date( $start) );
						break;
						case 'event_etime':
							$end = $EVENT->get_end_time();
							echo esc_attr( $EVENT->get_readable_formatted_date( $end) );
						break;
						case 'event_link':
							echo esc_url( $EVENT->get_permalink($event_ri, esc_attr( $lang ) ) );
						break;
						case 'event_subtitle':
							echo wp_kses_post( $EVENT->get_subtitle() );
						break;
						case 'event_location_name':
							echo esc_attr( $EVENT->get_location_name() );
						break;
						case 'event_location_add':
							echo wp_kses_post( $EVENT->get_location_address() );
						break;
			...
			echo "<div class='". esc_attr( $_cal_classes_string ) ."' >";
			echo "<div class='evo-data' " . esc_attr( $ev_excerpt ) . " ". esc_attr( $ev_expand )." data-ux_val='". esc_attr( $ev_uxval )."' data-exturl='" . esc_url( $external_url ). "' data-mapscroll='". esc_attr( $evcal_gmap_scrollw )."' data-mapformat='". esc_attr( $evcal_gmap_format )."' data-mapzoom='". esc_attr( $evcal_gmap_zooml )."' ></div><div class='evo_cal_data' data-sc='". wp_json_encode($SC)."'></div>";
