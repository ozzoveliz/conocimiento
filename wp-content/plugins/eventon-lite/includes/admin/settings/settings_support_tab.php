	...
	<div class="inside eventon_settings_page">
		<div class='evotrouble_left'>
			<div class='evotrouble_left_in' style='padding-right:10px; padding-top:20px'>
				<h3><?php esc_html_e('Common Issues and Solutions','eventon');?></h3>
				<p><?php esc_html_e('Find solutions to common issues with eventON lite from the below section.','eventon');?></p>
...
<?php
	...
	foreach($qas as $section=>$questions){
		echo '<h4>'. wp_kses_post( $section ).'</h4>';
		foreach($questions as $question=>$answer){
			echo '<h5>'.esc_html( $question ).'</h5><p style="display:none">'. wp_kses_post( $answer ).'</p>';
		}
	}
...
?>
		...
		<div class='evotrouble_right' style='text-align:center'>
			...
			<div class='evotrouble_documentation'>
				...
				<p style=' margin-bottom:25px; text-align:center'><i><?php esc_html_e('NOTE: Please feel free to type in your question and search our documentation library for related answeres','eventon') ;?></i></p>
			</div>
			<!-- video tutorials -->
				<a href='https://www.youtube.com/playlist?list=PLj0uAR9EylGrROSEOpT6WuL_ZkRgEIhLq' target='_blank' class='evo_admin_btn btn_prime'><?php esc_html_e('Video Tutorials on using EventON','eventon');?></a>
				

			<div class='evotrouble_bottom' style='padding-top:60px; text-align:center'>
				<div class='evo_support_box evo_troubleshoot'>
					<h2><?php esc_html_e('Having issues with EventON?','eventon');?> <br/>
						<a style='margin-top:8px; display:inline-block' class='evo_admin_btn' href='http://docs.myeventon.com/documentations/check-eventon-working/' target='_blank'><?php esc_html_e('Troubleshoot Guide to Eventon','eventon') ;?></a></h2>
					<p>Read our <a href='http://docs.myeventon.com/documentations/check-eventon-working/' target='_blank'><?php esc_html_e('troubleshooting guide','eventon');?></a> <?php esc_html_e('and identify your issue and apply common solutions to solve the issues before contacting us.','eventon') ;?></p>
				</div>
						

				<a class='evo_support_box twitter' href='http://www.twitter.com/myeventon' target='_blank'>
					<h3><?php esc_html_e('Follow us on twitter @myeventon','eventon');?></h3>
					<p><?php esc_html_e('You can get the latest updates, other news, tips and tricks for eventON via our twitter stream.','eventon');?></p>
				</a>
			</div>
		</div>
		<div class="clear"></div>
