...
<div id="evcal_4" class="postbox evcal_admin_meta curve" style='overflow: hidden;padding:20px'>		
	...
	<div class='evo_extend_page inside'>		
		...
		<div class='evo_ft_compare'>
			...
			<div class=''>
				<p><a href='https://myeventon.com/lite' target='_blank'><?php esc_html_e('Extend the features of EventON Lite with EventON Full','eventon');?> <i class='fa fa-up-right-from-square'></i></a></p>
				<h3 style='    text-transform: uppercase;font-size: 36px; font-weight: 900;'><?php esc_html_e('Compare Calendars','eventon');?></h3>
			</div>

			<div class='evoft_row fixed_top'>
				<div class='evoft_cell' style='background-color: #42c2f3'>
					<h3><a href='https://myeventon.com' target='_blank'><?php esc_html_e('EventON','eventon');?></a> <?php esc_html_e('Full Version','eventon');?></h3>
				</div>
				<div class='evoft_cell' style='background-color: #ffd548'>
					<h3><a href='https://myeventon.com/lite' target='_blank'><?php esc_html_e('EventON Lite','eventon');?></a></h3>
				</div>
			</div>
			...
			array('<b>USD$ 25</b> for life via <a href="https://codecanyon.net/item/eventon-wordpress-event-calendar-plugin/1211017">codecanyon</a>','Free for life','Pricing'),
							
		) as $row){
				if( empty($row[0])) continue;
				?>
				<div class='evoft_row'>
					<div class='evoft_header <?php echo isset($row[2])?'rowheader_style':'';?>'><span><?php echo isset($row[2])? wp_kses_post( $row[2] ):'';?></span></div>
					<div class='evoft_cell evoft_cell_1'><span><?php echo wp_kses_post( $row[0] );?></span></div>
					<div class='evoft_cell'><span><?php echo $row[1] == 'same'? wp_kses_post( $row[0] ): wp_kses_post( $row[1] );?></span></div>
				</div>
				<?php
			}
