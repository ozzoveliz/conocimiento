...
class evo_settings_settings{
			...
			array(
				...
				'fields'=> apply_filters('eventon_settings_general', array(
					...
					array(
					    ...
					    'type' => 'note',
					    'name' => sprintf(
					        __('Experience the complete EventON features with <a href="%s">EventON Full version.</a>', 'eventon'),
	...
	// HTML for eventcard designer
		function eventcard_meta_fields(){
			...
			?>
			<div class='evo_card_designer' data-dc='<?php echo esc_attr( $default_evc_color );?>'>
				<div class='evocard_design_holder'>
					...
					foreach($evo_ecl as $R=>$boxes){
						...
						echo "<p class='ecd_row". esc_attr( $L )."' data-r='". esc_attr( $R ) ."'><span class='ecd_row_in'>". $CC ."</span><i class='fa fa-minus-circle ecd_del_row'></i></p>";
					}	
				...
				<input type='hidden' id='evo_card_fields' value='<?php echo wp_json_encode($evo_ecl);?>' name='evo_ecl'/>
					...
					<div id='evo_card_field_selector_f'>
						...
						if( is_array($unused_fields) && count($unused_fields)>0){
							foreach($unused_fields as $ff){
								...
								echo "<span data-n='". esc_attr($ff)."'>". esc_html( $event_card_fields[$ff][1] ) ."</span>";
							}
						}
						?>
					</div>
					<p class='nothing' style='<?php echo count($unused_fields) > 0 ? "display:none":'';?>'><?php esc_html( _e('You are using all the available fields','eventon') );?>!</p>
	...
	function content_shortcodes(){
		...
		?>
			...
			<p><?php esc_html_e('[add_eventon] -- Default month calendar','eventon');?></p>
			<p><?php esc_html_e('[add_eventon_list number_of_months="5" hide_empty_months="yes" ] -- 5 months events list with empty months hidden from view','eventon');?></p>
			<p><?php esc_html_e('[add_eventon_list number_of_months="5" month_incre="-5" ] -- Show list of 5 past months','eventon');?></p>
			<p><i><?php esc_html_e('NOTE: For more shortcode examples and usage please visit demo.myeventon.com','eventon');?></i></p>
