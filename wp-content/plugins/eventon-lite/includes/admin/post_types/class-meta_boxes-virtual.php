...
<div class='evcal_data_block_style1 event_virtual_settings'>
	...
	<p class='yesno_row evo single_main_yesno_field'>
		...
		EVO()->elements->print_yesno_btn(array(
			'id'=>		'_virtual', 
			'var'=>		esc_attr( $EVENT->get_prop('_virtual') ),
			'input'=>	true,
			'attr'=>	array('afterstatement'=>'evo_virtual_details')
		));
		?>						
		<label class='single_yn_label' for='_virtual'><?php esc_html_e('This is a virtual (online) event', 'eventon')?></label>
	</p>
	...
	<div id='evo_virtual_details' class='evo_edit_field_box' style='display:<?php echo $EVENT->check_yn('_virtual')?'block':'none';?>'>
		...
		$btn_data = array(
			'lbvals'=> array(
				...
				't'=> esc_html__('Configure Virtual Event Details','eventon'),
				'ajax'=>'yes',
				'd'=> array(					
					'eid'=> esc_attr( $EVENT->ID ),
		...
		?>
		<p><span class='evo_btn evolb_trigger' <?php echo $this->helper->array_to_html_data($btn_data);?>  style='margin-right: 10px'><?php esc_html_e('Configure Virtual Event Details','eventon');?></span></p>		
	</div>									
	</div>									
</div>