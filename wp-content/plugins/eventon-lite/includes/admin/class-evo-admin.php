...
// admin init
	function eventon_admin_init() {				
		...
		// evneton settings only 
			...
			$admin_page = (isset($_GET['page']) )? sanitize_text_field( $_GET['page'] ): false;

			if($pagenow =='admin.php' && $admin_page && str_contains( $admin_page, 'eventon') ){
...
// admin menus
	function eventon_admin_menu() {
		...
		$main_page = add_menu_page(
			__('EventON Lite - Event Calendar','eventon'), 
			__('EventON Lite','eventon'),
			'manage_eventon',
			'eventon',
		...
		add_submenu_page( 
			'eventon', 
			__('EventON Lite - Language','eventon') , __('Language','eventon') , 
			'manage_eventon', 'eventon-lang', 
			array($this,'eventon_settings_page') 
		);
		add_submenu_page( 
			'eventon', 
			__('EventON Lite - Styles','eventon') , __('Styles','eventon') , 
			'manage_eventon', 'eventon-styles', 
			array($this,'eventon_settings_page') 
		);
		add_submenu_page( 
			'eventon', 
			__('EventON Lite - Extend','eventon') , __('Extend','eventon') , 
			'manage_eventon', 'eventon-extend', 
			array($this,'eventon_settings_page') 
		);
		add_submenu_page( 
			'eventon', 
			__('EventON Lite - Support','eventon') , __('Support','eventon') , 
			'manage_eventon', 'eventon-support', 
			array($this,'eventon_settings_page') 
		);
	}	
...
// admin styles and scripts
	// EVENTON Posts pages
		function eventon_admin_post_css() {
			...
			// JQ UI styles
			wp_enqueue_style("jquery-ui-css", AJDE_EVCAL_URL. '/assets/lib/jquery-ui/jquery-ui.css','',EVO()->version);
	...
	// scripts and styles for wp-admin
		function wp_admin_scripts_styles(){
			...
			// wp-admin styles
			 	...
			 	wp_enqueue_style( 'evo_wp_admin_lite',AJDE_EVCAL_URL.'/assets/css/admin/wp_admin_lite.css',array(), EVO()->version);
...
// Shortcode on Editor
	function eventon_shortcode_button_init() {
		...
		// load shortcode generator scripts into backend pages
